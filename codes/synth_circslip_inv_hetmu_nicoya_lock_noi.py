...
if slip_pattern == "circle":
    ...
    if pattern_option == 1:
        ...
        # convert to relative locations in km
        x0_tmp, y0_tmp = ut.azi_equidist_proj(lon0_slip, lat0_slip, lon0, lat0)
        x0_slip = np.round(x0_tmp)   # x center in km (integer)
        y0_slip = np.round(y0_tmp)   # y center in km (integer)
...
if not BOUNDED:
    # gammas_s = [2e2, 3e2, 4e2, 1e3]
    gammas_s = [1e4, 5e3, 1e3, 5e2]
