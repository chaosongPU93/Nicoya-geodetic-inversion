# %% [markdown]
# # Back-slip inversion with direction constrained to plate motion azimuth (1D depth-layered mu)
#
# Based on slip_inv_hetmu3D_nicoyaCK_locking_both_az.py — sibling that uses the
# DeShon et al. (2006) 1D, depth-layered shear modulus reference model directly
# (no 3D lateral variation, no scaling). Built via ut.build_1d_shear_modulus.
#
# Because the 1D model is piecewise-constant in depth, CG1 is sufficient
# (no edge-midpoint DOFs needed) and the filename does NOT carry a _CG suffix:
#   mu_str_het = "_DeShon1Dref"  →  outputs like  ..._DeShon1Dref.{xdmf,txt}
#
...
def solveCoseismicInversion_AzSlip(k, targets, m0_amp_expr, mtrue_mu_expr_inv,
    ...
    # If mtrue_mu_expr_inv is None, use the 1D depth-layered velocity model to compute shear modulus
...
# contrast_factor unused for the 1D depth-layered model — kept here as a stub for
# parity with the 3D / 3Dori siblings (no scaling applied).
# contrast_factor = 1.0  # amplification factor, original
...
print( "Converted from 1D velocity model to shear modulus, mu_str_het = ", mu_str_het)
...
#           set RUN_LCURVE = True  to run the 1D Het L-curve
# ============================================================
...
# gammas_s = [2e3, 3e3, 4e3] # az=45 sweep (previously)
gammas_s = [4e3, 3e3]   # two-point comparison at az=33.5° bracketing the az=45° L-curve corner

# ============================================================
# SINGLE TEST RUN (RUN_LCURVE = False): 1D μ (Het), large damping
# ============================================================
if not RUN_LCURVE:
    ...
    # 1D depth-layered model is piecewise constant — CG1 is sufficient (no edge-midpoint DOFs needed),
    # so no _CG{CG_mu_deg} suffix is appended to mu_str_inv.
    CG_mu_deg = 1
    print(sep, "SINGLE TEST RUN — 1D μ, Az-constrained scalar amplitude inversion", sep)
...
if RUN_LCURVE:
    # ============================================================
    # L-curve: 1D Het μ, Az-constrained amplitude
    # ============================================================
    print(sep, "1D Het L-curve — Az-constrained scalar amplitude inversion", sep)
    print(f"  azimuth_deg = {azimuth_deg:.1f}°,  amp_max = {amp_max*1e3:.1f} mm/yr,  use_tanh_amp = {use_tanh_amp}")

    dmisfit_1d = []
    mmisfit_1d = []
    # 1D model uses CG1 with no _CG suffix in mu_str_inv → the L-curve filename is
    # constant across iterations, so compute it once before the loop.
    mu_str_inv = mu_str_het
    ...
    for gamma_s in gammas_s:
        ...
        mtrue_mu_expr_inv = mtrue_mu_expr_het   # = None → 1D model inside solver
        mu_str_inv = mu_str_het
        # 1D depth-layered model is piecewise constant — CG1 is sufficient (no edge-midpoint DOFs needed),
        # so no _CG{CG_mu_deg} suffix is appended to mu_str_inv.
        CG_mu_deg = 1
        ...
        dmisfit_1d.append(misfit_d)
        mmisfit_1d.append(misfit_m)
        del results
        gc.collect()
    ...
    print("All 1D Het Az Inversion Finished!")
