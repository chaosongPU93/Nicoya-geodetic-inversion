...
if slip_pattern == "checker":
    ...
    if slip_azimuth_deg == 45.0:
        ...
        amp = np.sqrt(V_norm**2+V_para**2)  # about 83 mm
    elif slip_azimuth_deg == 33.5:
        amp = np.sqrt(V_norm**2+(V_para-V_para_const)**2)  # about 80 mm
...
if BOUNDED:
    ...
    #   dip:    [0, 1.1*amp] — 10% headroom above V_norm so tanh gradient stays non-zero
    #                          at fully-unlocked patches; truth ~0.994*amp ≈ fully-unlocked
...
# file identifier — '_azbd' suffix marks Az-class bounds (±amp strike, 1.1*amp dip)
if BOUNDED:
