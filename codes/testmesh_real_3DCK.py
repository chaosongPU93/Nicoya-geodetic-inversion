...
meshname = "nicoyaCK3_dense2_all"  # workable, checked with TWB, but maybe not fine enough
# meshname = "nicoyaCK3_dense7_all"  # save as v2, finer
...
def solveCoseismicInversion_TanhSlip(k, targets, m0_s_expr, mtrue_mu_expr_inv, gamma_val_H1, delta_val_L2, \
    ...
    if mtrue_mu_expr_inv == None:
        # interpolate and project the hybrid 3D and 1D velocity models to mesh to obtain shear modulus 
        #Original way: sharp edge at the boundary between the 3D and 1D models 
        vs_func, den_func, mu_func, _ = ut.process_velocity_models(vel3d, vel1d, den1d, mesh, verbose=False)

        # #Updated way: add a smooth transition at the boundary between the 3D and 1D models
        # vs_func, den_func, mu_func, _ = ut.process_velocity_models_v2(vel3d, vel1d, den1d, mesh, 
        #                                                               transition_width=25e3, verbose=False)
