...
def solveCoseismicInversion(k, targets, m0_s_expr, mtrue_mu_expr_inv, gamma_val_H1, delta_val_L2, \
    ...
    if pollute:
        if pollute_type == "uniform":
            ...
            # # If sigma_e ≈ sigma_n, then sigma_xy ≈ 0 and you can use:
            # obs_weights[9::15] = 2 / (data['vx_std_Car']**2 + data['vy_std_Car']**2).to_numpy() / (f_h**2)
            # obs_weights[10::15] = 2 / (data['vx_std_Car']**2 + data['vy_std_Car']**2).to_numpy() / (f_h**2)
...
# rho_s = 1e9
# rho_s = 1e8 # for a correlation length of 20 km
# rho_s = 1e7

rhos_s = [1e6, 5e6, 1e7, 1e8]
# rhos_s = [4e6, 6e6, 8e6]
...
print( sep, "L-curve computation for slip with gamma_s fixed", sep )
...
for rho_s in rhos_s:
    ...
    mu_str_inv = mu_str_het
    print("Solving inverse problem based on: ", mu_str_inv)
    solveCoseismicInversion(k, targets, m0_s_expr, mtrue_mu_expr_inv, gamma_s, delta_s, \
                            pollute=pollute, pollute_type=pollute_type, savefiles=True, verbose=True)
    print("Het Inversion finished!!!")
