# %% [markdown]
# # Synthetic forward (Az-consistent) — STRIPE, 3D
#
# Based on: synth_stripeslip_az_scalaramp_inv_hetmu3D_uneven_nicoyaCK_lock_noi.py
#
# PURPOSE: Test the AzSlip-consistent forward scheme with 3D DeShon velocity model.
#          Same operator mismatch fix as the Hom version, but with CG2 shear modulus.
#
# SCHEME: AzSlip-consistent forward
#   True slip: m_phys(x) = amp in locked patches, 0 in creeping (scalar, CG1)
#   Forward:   PDEVarf_AzSlip_Direct_3D — slip = m_phys * (c_strike_fun, c_dip_fun)
#              mu = self.mtrue_mu_fun directly (already in GPa, from process_velocity_models_hull)
#   Inversion: PDEVarf_AzSlip — same convention (mu already in GPa)
#
# OUTPUTS (prefixed d_obs_cf_* and _cf_ in inv_str):
#   caz_diag_<meshname>_az<deg>.txt
#   slip_true_cf_<meshname>...txt
#   d_obs_cf_<meshname>...txt
#   d_obs_grid_cf_<meshname>...txt
#   u_cf_<meshname>...xdmf
...
sep = "\n"+"#"*80+"\n"
print(sep, "START of Az-consistent forward (stripe, 3D DeShon)", sep)
...
def save_function_with_dofs(function, filename_base, attr_name, cg_degree=1):
    ...
    xdmf_file.close()   # CRITICAL: leaking handle hangs MUMPS at next L-curve iteration
    if cg_degree > 1:
...
# CRITICAL for 3D: mu = self.mtrue_mu_fun directly (already in GPa, dl.Function)
# ============================================================================
...
# %%
# ============================================================================
# AZ-CONSISTENT FORWARD: STATION OBSERVATIONS (3D version)
# ============================================================================
def solveCoseismicForward_AzConsistent(k, targets, mtrue_mu_expr_for,
    ...
    if mtrue_mu_expr_for is None:
        ...
        # mtrue_mu_fun is a dl.Function, already in GPa
    else:
    ...
    if savefiles:
        ...
        uid.write(u_save)
        uid.close()   # CRITICAL: leaking handle hangs MUMPS at next L-curve iteration
...
# %%
# ============================================================================
# AZ-CONSISTENT FORWARD: DENSE GRID (3D version)
# ============================================================================
...
# CRITICAL for 3D: mu = self.mtrue_mu_fun directly (already in GPa)
# ============================================================================
...
# %%
# ============================================================================
# INVERSION: Az-constrained scalar amplitude (3D version)
...
if RUN_LCURVE:
    ...
    # gammas_s_rest     = [4e1, 6e1, 8e1, 3e2, 5e2, 8e2, 1e3]
    gammas_s_rest     = [5e3, 1e4]
    ...
    _passes = [
        ...
        #  mu_str_hom, 1),
        ("Het 3D L-curve (rest)",     gammas_s_rest,     outFileName_het, mtrue_mu_expr_het,
         mu_str_het + f"_CG{CG_mu_deg}", CG_mu_deg),
        ("Hom L-curve (rest)",        gammas_s_rest,     outFileName_hom, mtrue_mu_expr_hom,
         mu_str_hom, 1),
    ]
