# %% [markdown]
# # Back-slip inversion with ULTRA-FAST TAO BOX CONSTRAINTS (Maximum Performance)
# 
# This ULTRA-FAST version implements aggressive performance optimizations:
# * Parallel linear algebra operations (use all available cores)
# * Aggressive caching and memoization
# * Reduced tolerance for faster convergence
# * Memory pre-allocation and efficient data structures
# * Simplified solver configurations for speed
# * Early termination strategies
# 
# Trade-offs: Higher memory usage, more CPU cores, potentially less accuracy for speed
# 
# Based on: slip_inv_hetmu_nicoyaCK_locking_both_tao_optimized.py

# %%
# ULTRA-FAST: Use ALL available threads and cores
import sys, os
import multiprocessing
n_cores = 5
print(f"Setting threads to use ALL {n_cores} cores for maximum speed")
...
# ULTRA-FAST: Aggressive compiler optimizations
dl.parameters["form_compiler"]["quadrature_degree"] = 3  # Reduced from 5 for speed
dl.parameters["form_compiler"]["optimize"] = True
dl.parameters["form_compiler"]["cpp_optimize"] = True
dl.parameters["form_compiler"]["cpp_optimize_flags"] = "-O3 -ffast-math"

# Mute ALL warnings for speed
import logging
...
# %%
# ULTRA-FAST TAO BOX-CONSTRAINED PDE Variational Formulation
class PDEVarf_TAOUltraFast:
    """
    Ultra-fast TAO box-constrained PDE variational formulation
    ...
    - Direct slip parameters (no transformation)
    """
...
# %%
# ULTRA-FAST TAO Objective Function Class for hipPYlib Model
class UltraFastTAOObjectiveFunction:
    """
    ULTRA-FAST TAO-compatible objective function wrapper for hipPYlib models
    
    Maximum speed optimizations:
    - Aggressive caching with larger cache size
    - Pre-allocated memory pools
    - Simplified computations where possible
    - Early termination strategies
    - Reduced precision for speed
    """
    def __init__(self, model, Vh):
        self.model = model
        self.Vh = Vh
        ...
        self.grad_vec = model.generate_vector(hp.PARAMETER)
        
        # ULTRA-FAST: Larger cache with multiple slots
        self.cache_size = 5
        self.cache = {}
        self.cache_order = []
        ...
        # ULTRA-FAST: Pre-compute frequently used arrays
        self.temp_arrays = {
            'param_copy': np.zeros(len(self.mg)),
            'grad_copy': np.zeros(len(self.mg))
        }
        
    def _compute_param_hash(self, m_array):
        """Ultra-fast hash for parameter detection"""
        # Use a subset of parameters for faster hashing
        stride = max(1, len(m_array) // 100)  # Sample every 100th element
        return hash(m_array[::stride].tobytes())
    
    def _update_parameter_vector(self, m_petsc):
        """Ultra-fast parameter vector update"""
        m_array = m_petsc.getArray()
        # Reuse pre-allocated array
        self.temp_arrays['param_copy'][:] = m_array
        self.mg.set_local(self.temp_arrays['param_copy'])
        self.mg.apply('')
        return m_array
        
    def _solve_forward_adjoint(self, x):
        """Ultra-fast forward and adjoint solve"""
    ...
    def _manage_cache(self, param_hash, cost, gradient):
        """Ultra-fast cache management with LRU"""
        ...
        self.cache_order.append(param_hash)
    
    def _compute_if_needed(self, m_petsc):
        """Ultra-fast computation with aggressive caching"""
        # Update parameter vector efficiently
        m_array = self._update_parameter_vector(m_petsc)
        
        # Check cache first
        current_hash = self._compute_param_hash(m_array)
        if current_hash in self.cache:
            self.cache_hits += 1
            cached = self.cache[current_hash]
            self.last_cost = cached['cost']
            self.last_gradient = cached['gradient']
            return
        
        self.eval_count += 1
        
        # Create state vector
        x = [self.u, self.mg, self.p]
        
        # SINGLE SOLVE: Solve forward and adjoint problems once
        self._solve_forward_adjoint(x)
        
        # Compute objective function
        cost = self.model.cost(x)[0]  # total cost
        
        # Compute gradient in same function call
        self.model.evalGradientParameter(x, self.grad_vec)
        gradient_array = self.grad_vec.get_local()
        
        # Cache results
        self._manage_cache(current_hash, cost, gradient_array)
        self.last_cost = cost
        self.last_gradient = gradient_array
        
        # Print progress every N evaluations
        if self.eval_count % 5 == 0:  # More frequent progress updates
            avg_solve_time = self.solve_time / self.eval_count
            cache_rate = 100 * self.cache_hits / (self.eval_count + self.cache_hits)
            print(f"TAO eval {self.eval_count}: cost = {cost:.4e}, "
                  f"cache rate = {cache_rate:.1f}%, avg solve = {avg_solve_time:.2f}s")
    
    def formObjective(self, tao, m_petsc):
        """Ultra-fast TAO callback for objective evaluation"""
        try:
            self._compute_if_needed(m_petsc)
            return self.last_cost
        except Exception as e:
            print(f"Error in TAO objective evaluation: {e}")
            raise
    
    def formGradient(self, tao, m_petsc, g_petsc):
        """Ultra-fast TAO callback for gradient evaluation"""
        try:
            self._compute_if_needed(m_petsc)
            g_petsc.setArray(self.last_gradient)
        except Exception as e:
            print(f"Error in TAO gradient evaluation: {e}")
            raise

# %%
# ULTRA-FAST TAO Box-Constrained Inversion Function
def solveCoseismicInversion_TAOUltraFast(k, targets, m0_s_expr, mtrue_mu_expr_inv, gamma_val_H1, delta_val_L2, \
                            box_transformer, savefiles=True, verbose=True):
    """
    ULTRA-FAST TAO box-constrained slip inversion using maximum performance optimizations
    ...
    Returns:
        Same outputs as original function, but much faster
    """
    ...
    if verbose:
        print(sep, f"ULTRA-FAST TAO BOX-CONSTRAINED: Using {n_cores} cores for maximum speed", sep)
        print("Number of dofs: STATE={0}, PARAMETER={1}, ADJOINT={2}".format(*ndofs))
        print("Number of STATE and ADJOINT dofs: STRESS={0}, DISPLACEMENT={1}, ROTATION={2}".format(*ndofs_state))
        print("")
        print("ULTRA-FAST TAO BOX CONSTRAINT FRAMEWORK")
        print(f"   {box_transformer}")
        print("   Speed optimizations: Relaxed tolerances, aggressive caching, parallel processing")
        
        if box_transformer.has_any_bounds:
            print("   Parameter interpretation:")
            print("     m = physical_slip (DIRECT, no transformation)")
            print("     Bounds enforced by TAO TAOBLMVM solver")
        else:
            print("   Using unconstrained TAO solver")
    ...
    # Use ultra-fast TAO box-constrained PDE variational formulation
    pde_varf = PDEVarf_TAOUltraFast(mtrue_mu_fun)
    if verbose:
        print("Using ultra-fast TAO box-constrained PDE (no slip transformation)")

    # Define the PDE problem
    pde = hp.PDEVariationalProblem(Vh, pde_varf, bc, bc0, is_fwd_linear=True)

    # ULTRA-FAST: Optimize solver settings for speed
    type_solver = "mumps"  # Fast direct solver
    ...
    # ULTRA-FAST: Set aggressive solver parameters for speed
    solver_params = {
        "linear_solver": "mumps",
        "preconditioner": "none",
        "mumps_icntl_14": 100,  # Increase memory for speed
        "mumps_icntl_23": 2000,  # Maximum memory per core
        "mumps_cntl_1": 0.01,   # Relative pivot threshold (faster)
    }
    
    for solver in [pde.solver, pde.solver_fwd_inc, pde.solver_adj_inc]:
        for key, val in solver_params.items():
            try:
                solver.parameters[key] = val
            except:
    ...
    # ULTRA-FAST TAO SOLVER SETUP
    if verbose:
        print(sep, "Setting up ULTRA-FAST TAO box-constrained solver", sep)
    ...
    # Set TAO algorithm
    if box_transformer.has_any_bounds:
        tao.setType(PETSc.TAO.Type.BLMVM)  # Box-constrained L-BFGS
        if verbose:
            print("Using TAO BLMVM (box-constrained L-BFGS)")
    else:
        tao.setType(PETSc.TAO.Type.LMVM)   # Unconstrained L-BFGS
        if verbose:
            print("Using TAO LMVM (unconstrained L-BFGS)")
    
    # Create PETSc vectors for TAO
    # Convert hipPYlib vector to PETSc Vec
    m_petsc = PETSc.Vec().create(comm=PETSc.COMM_WORLD)
    m_petsc.setSizes(len(m))
    m_petsc.setFromOptions()
    m_petsc.setUp()
    m_petsc.setArray(m.get_local())
    
    # Set bounds if constrained
    if box_transformer.has_any_bounds:
        # Create bound vectors
        lb_petsc = m_petsc.duplicate()
        ub_petsc = m_petsc.duplicate()
        
        # Apply constraints to fault interface DoFs
        fault_indices = np.where(fault_mask)[0]
        
        if box_transformer.has_strike_bounds or box_transformer.has_dip_bounds:
            # Create bounds arrays
            lb_values = np.full(len(m), -PETSc.INFINITY)
            ub_values = np.full(len(m), PETSc.INFINITY)
            
            for i in fault_indices:
                component_type = i % 2  # 0 = strike, 1 = dip
                
                if component_type == 0 and box_transformer.has_strike_bounds:
                    # Strike component
                    lb_values[i] = box_transformer.strike_min
                    ub_values[i] = box_transformer.strike_max
                elif component_type == 1 and box_transformer.has_dip_bounds:
                    # Dip component  
                    lb_values[i] = box_transformer.dip_min
                    ub_values[i] = box_transformer.dip_max
            
            # Set the arrays to PETSc vectors
            lb_petsc.setArray(lb_values)
            ub_petsc.setArray(ub_values)
        
        # Set bounds in TAO
        tao.setVariableBounds(lb_petsc, ub_petsc)
        
        if verbose:
            n_constrained = len(fault_indices)
            print(f"Applied bounds to {n_constrained} fault interface DoFs")
    
    # Create ULTRA-FAST objective function
    objective = UltraFastTAOObjectiveFunction(model, Vh)
    
    # Set TAO objective and gradient separately (but optimized with caching)
    tao.setObjective(objective.formObjective)
    tao.setGradient(objective.formGradient)
    
    # ULTRA-FAST: Relaxed tolerances for faster convergence
    tao.setTolerances(gatol=1e-6, grtol=1e-6, gttol=1e-9)  # Relaxed from 1e-9, 1e-9, 1e-12
    tao.setMaximumIterations(500)  # Reduced from 1000
    
    # ULTRA-FAST: Set aggressive TAO parameters
    tao.setFromOptions()
    
    # Override with speed-focused parameters
    tao_opts = PETSc.Options()
    tao_opts.setValue("-tao_ls_type", "more-thuente")  # Faster line search
    tao_opts.setValue("-tao_ls_ftol", "0.05")  # Relaxed line search tolerance
    tao_opts.setValue("-tao_ls_gtol", "0.9")   # Relaxed gradient tolerance
    tao_opts.setValue("-tao_max_funcs", "10")  # Limit function evaluations per iteration
    
    if verbose:
        print("ULTRA-FAST TAO solver configured successfully")
        print("Features: Relaxed tolerances, aggressive caching, maximum parallelism")
        print("Starting optimization...")
    
    # SOLVE with ULTRA-FAST TAO
    start_time = time.time()
    tao.solve(m_petsc)
    optimization_time = time.time() - start_time
    
    # Get TAO solution info
    converged_reason = tao.getConvergedReason()
    iterations = tao.getIterationNumber()
    function_evals = tao.getFunctionEvaluations()[0]
    gradient_evals = tao.getGradientEvaluations()[0]
    
    if verbose:
        print(f"ULTRA-FAST TAO optimization completed in {optimization_time:.2f} seconds")
        print(f"Converged reason: {converged_reason}")
        print(f"Iterations: {iterations}")
        print(f"Function evaluations: {function_evals}")
        print(f"Gradient evaluations: {gradient_evals}")
        print(f"Total objective+gradient evaluations: {objective.eval_count}")
        print(f"Cache hits: {objective.cache_hits}")
        total_evals = objective.eval_count + objective.cache_hits
        print(f"Cache hit rate: {100*objective.cache_hits/total_evals:.1f}%")
        print(f"Average solve time: {objective.solve_time/objective.eval_count:.3f}s")
        print(f"Speed improvement vs original: ~{2123*60/optimization_time:.1f}x faster")
    
    # Update solution vector
    m_solution = m_petsc.getArray()
    m.set_local(m_solution.copy())
    m.apply('')
...
# %%
# TAO BOX CONSTRAINT SETUP
print(sep + "TAO BOX CONSTRAINT SETUP" + sep)
...
if BOUNDED:
    ...
    s_strike_max = 0.027 - 0.011  # remove the constant 11 mm max along-strike slip
...
# file identifier
if BOUNDED:
    inv_str = f"_lockbothtaobd_w{w_h}{w_v}_gs{gamma_val_H1:.0e}_ds{delta_val_L2:.0e}"
else:
...
print( sep, "L-curve computation with TAO box constraints", sep )
...
for gamma_s in gammas_s:
    ...
    # file identifier
    if BOUNDED:
        inv_str = f"_lockbothtaobd_w{w_h}{w_v}_gs{gamma_s:.0e}_ds{delta_s:.0e}"
    else:
    ...
    # Solve slip inverse problem within the homogeneous shear modulus half-space WITH TAO BOX CONSTRAINTS
    mtrue_mu_expr_inv = mtrue_mu_expr_hom
    mu_str_inv = mu_str_hom
    print("Solving inverse problem based on: ", mu_str_inv)
    results = solveCoseismicInversion_TAOUltraFast(
    ...
    print("Hom Inversion with TAO box constraints finished!!!")

print("All Inversion with TAO Box Constraints Finished!")   