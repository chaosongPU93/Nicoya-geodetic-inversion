# %% [markdown]
# # Synthetic recovery test — Az-constrained SCALAR amplitude inversion (CHECKERBOARD)
#
# Based on synth_checkslip_az_inv_hetmu_uneven_nicoyaCK_lock_noi2.py (reference).
# Checkerboard variant of synth_stripeslip_az_scalaramp_inv_hetmu_uneven_nicoyaCK_lock_noi2.py.
#
...
#   - Forward:     identical to reference (solveCoseismicForward)
...
# %%
# Forward shear modulus (homogeneous for synthetics, as in reference)
mu_b = 0; mu_lower = mu_expr_fn(mu_b); mu_upper = mu_expr_fn(mu_b)
...
# (synth_stripeslip_az_inv_hetmu_uneven_nicoyaCK_lock_noi2.py with same parameters)
...
# ============================================================
# SINGLE TEST RUN — Hom μ, large damping, Az scalar amplitude
# ============================================================
if not RUN_LCURVE:
    gamma_s_test = 1e3
    ...
    # No masking needed: zero patches should recover near 0.
...
# %%
# ============================================================
# L-curve: Hom μ, Az scalar amplitude
# ============================================================
if RUN_LCURVE:
    # gammas_s = [5e1, 1e2, 2e2, 4e2, 6e2, 8e2]
    # gammas_s = [1e1, 4e2, 6e2, 8e2]
    gammas_s = [5e1, 1e2, 2e2, 4e2, 8e2, 1e3]
    gammas_s = gammas_s[::-1]

    print(sep, "Hom L-curve — Az scalar amplitude inversion", sep)

    outFileName = f"Lcurvesynscaamp_azbd_rs{rho_s:.0e}_{meshname}_{slip_pattern}_{pattern_option}_{mu_str_for}_{mu_str_hom}.txt"
    csvoutput = open(resultpath + outFileName, 'a')
    for gamma_s in gammas_s:
        ...
        try:
            ...
            pass
    csvoutput.close()
    print("All Hom Az scalar amplitude inversions finished!")
