# %% [markdown]
# This is a duplicate of 'synth_stripeslip_inv_hetmu_nicoyaCK_lock_noi2.py' with limited changes to test
...
# Prescribed slip azimuth: geographic direction CW from North.
# The slip on each facet is the in-plane vector whose horizontal projection
# points along this azimuth.  For Nicoya, 45° (N45E) aligns with the mesh
# x-axis and is close to the average down-dip direction.
slip_azimuth_deg = 45.0   # degrees CW from North (e.g. 45 = N45E)
mesh_rot_deg     = 45.0   # CCW rotation used when building the mesh (Nicoya convention)

if slip_pattern == "checker":
    ##### CHECKERBOARD SLIP MODEL — uniform geographic azimuth #####
    print("Creating fault-local checkerboard (uniform azimuth) with natural depth range...")

    V_norm = 78.5 / 1e3     # the trench-normal long-term loading of 78.5 mm
    amp = V_norm   # interseismic coupling case, max == complete coupling of 1
    # amp = 1   # coseismic case, max == 1 m
    ...
    if pattern_option == 1:
        dx = 35e3   # half-wavelength along averaged strike (m)
        dy = 35e3   # half-wavelength along averaged dip (m)
        x0 = -20e3  # offset along averaged strike
        y0 = -60e3  # offset along averaged dip
        rot_deg = 45  # in-plane pattern rotation (CCW from averaged strike)
    ...
    mtrue_s_expr_gt = ut.create_fault_local_checkerboard_az(
        ...
        mesh_rotation_deg=mesh_rot_deg,
        x0=x0,
        y0=y0,
    ...
    slip_str_gt = (
        ...
        f"_rot{rot_deg:g}_az{slip_azimuth_deg:g}_ms{amp:g}"
    )


elif slip_pattern == "stripe":
    ##### STRIPE SLIP MODEL — uniform geographic azimuth #####
    print("Creating fault-local stripes (uniform azimuth) with natural depth range...")
    ...
    if pattern_option == 1:
        ...
        x_len = 80e3    # stripe width in averaged dip direction (m)
        y_len = 300e3   # stripe length in averaged strike direction (m)
        dx = 35e3       # gap between stripes (m)
