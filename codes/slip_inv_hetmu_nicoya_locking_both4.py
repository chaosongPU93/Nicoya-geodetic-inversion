...
def solveCoseismicInversion(k, targets, m0_s_expr, mtrue_mu_expr_inv, gamma_val_H1, delta_val_L2, savefiles=True, verbose=True):
    ...
    # ##Inject volcanoes
    # tmp_mu = mu_u*np.ones(CG_mu.dim(),)
    # for i in range(volc.shape[0]):
    #     # Assign a sphere with different value
    #     # set the center of the sphere at volcatno location but at -30 km depth, and radius 30 km, \mu inside the sphere is mu_v_VOLC
    #     mtrue_mu_volc_expr = dl.Expression(
    #         '( nu_d*(pow(x[0] - x_volc,2) + pow(x[1] - y_volc,2) + pow(x[2] - z_volc,2) < pow(r_mu,2) ) )', 
    #         x_volc=volc['x'].iloc()[i], y_volc=volc['y'].iloc()[i], z_volc=-30e3, nu_d=mu_v, r_mu=30e3, degree=5)
    #     # Interpolate 
    #     m_true_mu_volc = dl.interpolate(mtrue_mu_volc_expr, CG_mu)
    #     # Extract 
    #     idx_volc = list( np.nonzero(m_true_mu_volc.vector()[:])[0] )
    #     # Fill the array
    #     tmp_mu[idx_volc] = m_true_mu_volc.vector()[idx_volc].copy() 
    #     # Fill with volcanoes
    #     mtrue_mu[idx_volc] = tmp_mu[idx_volc] 
    ...
    return mtrue_mu, xf, yf, zf, m, u, m_fun, d_obs, d_cal, m_s_fault, misfitd, grad_m 
    # return mtrue_mu   
...
# file identifier
# inv_str = f"_w{w_h}{w_v}_gs{gamma_val_H1:.0e}_ds{delta_val_L2:.0e}"
