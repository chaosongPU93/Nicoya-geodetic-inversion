# %% [markdown]
# Duplicate of 'synth_stripeslip_inv_hetmu_uneven_nicoyaCK_lock_noi.py' with two key changes:
#   1. Slip prescription uses FaultLocalStripesAz (create_fault_local_stripes_az): slip direction
#      fixed to a geographic azimuth (default N45E = plate convergence) per facet, rather than
#      averaged local down-dip. Forward uses heterogeneous mu structure.
#   2. Inversion bounds updated for curved Nicoya fault:
#        strike_bounds = (-amp, +amp)      # ±78.5 mm/yr; ±16 mm/yr clips 35% of truth
#        dip_bounds    = (0.0, 1.1*amp)   # 10% headroom above V_norm
#
# L-curve is commented out (not needed for now). Two single-run inversions are active:
#   - Het inversion (mu matches forward): quantifies slip pattern recovery
#   - Hom inversion (mu mismatched intentionally): quantifies mu-model effect on recovery
# RAM cleanup (del results + gc.collect) is applied between the two single runs.
...
if slip_pattern == "checker":
    ...
    # Define true model: slip in fixed geographic azimuth, magnitude = stripe pattern
    V_norm = 78.5   # in mm
    V_para = 27
    V_para_const = 11
...
rho_s = 2e7
gamma_val_H1 = 3e3  # try if unbounded
...
#     gammas_s = [6e1, 1e2, 2e2, 2.5e2, 3e2, 3.5e2, 4e2, 4.5e2, 5e2, 6e2, 8e2, 1e3, 2e3]  # fine-tuned
...
#     if BOUNDED:
#         inv_str = f"_synlockbd_azbd_w{w_h}{w_v}_gs{gamma_s:.1e}_ds{delta_s:.1e}"
#     else:
#         inv_str = f"_synlock_az_w{w_h}{w_v}_gs{gamma_s:.1e}_ds{delta_s:.1e}"
#     print("Inverse problem identifier: ", inv_str)
...
# if BOUNDED:
#     outFileName = f"Lcurvesynlockbd_azbd_rs{rho_s:.0e}_{meshname}_{slip_pattern}_{pattern_option}_{mu_str_for}_{mu_str_inv}.txt"
# else:
#     outFileName = f"Lcurvesynlock_az_rs{rho_s:.0e}_{meshname}_{slip_pattern}_{pattern_option}_{mu_str_for}_{mu_str_inv}.txt"
