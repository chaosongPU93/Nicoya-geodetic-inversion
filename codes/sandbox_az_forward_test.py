"""
sandbox_az_forward_test.py
==========================
Sandbox to validate FaultLocalStripesAz / FaultLocalCheckerboardAz.

Runs a single forward solve with the NEW Az class and, for comparison,
re-runs with the OLD pure dip-slip class.  Analyses:
...
Uses the even-top mesh (nicoyaCKden_sm) to skip uneven mesh complexity.
Saves all outputs to a sandbox subdirectory, never touching existing results.

Run with:
    conda activate fenics
    python sandbox_az_forward_test.py
"""
...
# ---------------------------------------------------------------------------
# Slip parameters (stripe, option 1: 2-stripe shallow-deep)
# ---------------------------------------------------------------------------
V_norm        = 78.5 / 1e3
amp           = V_norm
x_len         = 80e3
...
slip_expr_old = ut.create_fault_local_stripes(
    mesh=mesh, boundaries=boundaries, fault_id=fault,
    amp=amp,
    stripe_width=x_len, stripe_spacing=stripe_spacing, stripe_length=y_len,
    x0=x0_pat, y0=y0_pat, rotation_deg=rot_deg,
    degree=5
)

# NEW: uniform azimuth (per-facet projection)
slip_expr_new = ut.create_fault_local_stripes_az(
    mesh=mesh, boundaries=boundaries, fault_id=fault,
    amp=amp,
    stripe_width=x_len, stripe_spacing=stripe_spacing, stripe_length=y_len,
...
# Forward solves
# ---------------------------------------------------------------------------
tag_old = f"{meshname}_stripe_OLD_az{slip_azimuth_deg:g}"
tag_new = f"{meshname}_stripe_AZ{slip_azimuth_deg:g}"
...
print("We verify using per-facet normals stored in the Az expression object.\n")

# Access the precomputed per-facet coefficients and reconstruct check
...
for facet_idx, (cs, cd) in facet_coeffs.items():
    # Retrieve the facet normal used during precomputation
    # (We cannot retrieve it easily post-hoc, so instead we verify using the
    # relationship: v_hat = cs * strike_vec + cd * dip_vec, which by construction
    # should have h as its horizontal projection when normalised.
    # Check instead: horizontal angle of slip vector using the expression's avg frame)
    pass

# Simpler verification: use the averaged fault frame stored in the expression
...
# For active DOFs in the NEW class, reconstruct slip vector using avg frame
# (approximation — exact reconstruction requires per-facet normals)
active_new = np.abs(s_dip_new) > 1e-10
if active_new.sum() > 0:
    # Approximate slip vectors using averaged frame
    slip_x = s_str_new[active_new] * avg_strike[0] + s_dip_new[active_new] * avg_dip[0]
    slip_y = s_str_new[active_new] * avg_strike[1] + s_dip_new[active_new] * avg_dip[1]
    slip_h_mag = np.sqrt(slip_x**2 + slip_y**2)
    # Avoid division by zero
    valid = slip_h_mag > 1e-10
    az_recon = np.rad2deg(np.arctan2(slip_x[valid], slip_y[valid]))   # CW from N
    print(f"\n  Reconstructed horizontal slip azimuth (approx, using avg frame):")
    ...
    print("  No active DOFs found for NEW class.")
...
print(sep + "SANDBOX COMPLETE" + sep)
