...
# %%
# Define the expression of slip within a bound [minval, maxval]
def m_s_expression(m, minval, maxval):
    scaled = (ufl.tanh(m) + 1) / 2  
    m_s = minval + (maxval - minval) * scaled
    return m_s


# %%
# The linear coseismic forward problem
...
# Define the weak formulation of the FORWARD problem
class PDEVarfINV:
    def __init__(self, mtrue_mu_fun, bounds):
        self.mtrue_mu_fun = mtrue_mu_fun
        self.bounds = bounds

    def __call__(self, u, m, p):
        sigma, uu, r = dl.split(u)
        m_strike_fun, m_dip_fun = dl.split(m)
        tau, w, q = dl.split(p)
        u0 = dl.Constant((0., 0., 0.))

        mu = mu_expression(self.mtrue_mu_fun)

        strike_min = self.bounds['strike_min']
        strike_max = self.bounds['strike_max']
        dip_min = self.bounds['dip_min']
        dip_max = self.bounds['dip_max']
            
        m_strike = m_s_expression(m_strike_fun, strike_min, strike_max)
        m_dip = m_s_expression(m_dip_fun, dip_min, dip_max)
...
def solveCoseismicForward(k, targets, mtrue_mu_expr_for, mtrue_s_expr=None, pollute=True, \
    ...
    mtrue.set_local(tmp)
    # print(mtrue[0:299:3])
    # csvoutput = open(resultpath + 'a.txt', 'w+')
    # for i in range(0, int(tmp.shape[0]/3)):
    #     csvoutput.write( "%.6f \n" %(mtrue[i*3]) )
    # csvoutput.close()
...
def solveCoseismicInversion(k, targets, m0_s_expr, m_s_bounds, mtrue_mu_expr_inv, gamma_val_H1, delta_val_L2, \
    ...
    # Define the PDE problem
    pde_varf = PDEVarfINV(mtrue_mu_fun, m_s_bounds)
    pde = hp.PDEVariationalProblem(Vh, pde_varf, bc, bc0, is_fwd_linear=True)
    ...
    CG_s = dl.FunctionSpace(mesh, "CG", 1)
    m_fun = hp.vector2Function(m, Vh[hp.PARAMETER])
    m_fun_strike, m_fun_dip = m_fun.split(deepcopy=True)
    m_fun_strike_expr = m_s_expression(m_fun_strike, m_s_bounds['strike_min'], m_s_bounds['strike_max'])
    s_strike_fun = dl.project( m_fun_strike_expr, CG_s )
    m_fun_dip_expr = m_s_expression(m_fun_dip, m_s_bounds['dip_min'], m_s_bounds['dip_max'])    
    s_dip_fun = dl.project( m_fun_dip_expr, CG_s )
    ...
    grad_m = dl.assemble(
        (ufl.inner(ufl.avg(ufl.nabla_grad(s_strike_fun)), ufl.avg(ufl.nabla_grad(s_strike_fun)))
         + ufl.inner(ufl.avg(ufl.nabla_grad(s_dip_fun)),    ufl.avg(ufl.nabla_grad(s_dip_fun)))) * dS(fault)
         )
    misfitd = np.linalg.norm((d_cal-d_obs), 2)
    print( "Data misfit {0:.6e}; Model misfit {1:.6e};".format(misfitd, grad_m) )
    # print( "Data misfit {0:.6e}".format(misfitd) )
    ...
    s_mag = dl.project(ufl.sqrt(s_strike_fun**2 + s_dip_fun**2), CG_s )
    ...
    um2_vals = np.array(um2.vector()[:])      # length = 22092
    mask_full = (um2_vals == 99)
    # Pick out only strike indices (0::2) and dip indices (1::2)
    mask_strike = mask_full[0::2]   # length = 11046
    mask_dip    = mask_full[1::2]   # length = 11046
    # Now both masks align with your scalar functions
    s_strike_vals = np.array(s_strike_fun.vector()[:])
    s_dip_vals    = np.array(s_dip_fun.vector()[:])
    m_sx_fault = s_strike_vals[mask_strike]
    m_sy_fault = s_dip_vals[mask_dip]
    print( min(m_sx_fault), max(m_sx_fault) )
    print( min(m_sy_fault), max(m_sy_fault) )
    # Interleave strike and dip into a 1D array
    m_s_fault = np.empty(m_sx_fault.size + m_sy_fault.size, dtype=m_sx_fault.dtype)
    m_s_fault[0::2] = m_sx_fault  # strike values at even indices
    m_s_fault[1::2] = m_sy_fault  # dip values at odd indices
...
# %%
# Define constraints based on your problem
V_para = 16/1e3
if slip_pattern == "checker" or slip_pattern == "stripe":
    slip_bounds = {
        'strike_min':0,
        'strike_max': V_para,
        'dip_min': 0.0,
...
print(f"Using slip bounds: {slip_bounds}")
...
_, _, _, _, m, _, _, _, _, _, _, _, _ = \
    solveCoseismicInversion(k, targets, m0_s_expr, slip_bounds, mtrue_mu_expr_inv, gamma_val_H1, delta_val_L2, \
                            pollute=pollute, pollute_type=pollute_type, savefiles=True, verbose=True)
...
# print("Solving inverse problem based on: ", mu_str_inv)
# solveCoseismicInversion(k, targets, m0_s_expr, mtrue_mu_expr_inv, gamma_val_H1, delta_val_L2, \
#                         pollute=pollute, pollute_type=pollute_type, savefiles=True, verbose=True)
