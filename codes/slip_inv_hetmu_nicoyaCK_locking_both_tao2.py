# %% [markdown]
# # Back-slip inversion with TAO BOX CONSTRAINTS (PETSc native)
# 
# This version implements direct box-constrained optimization using TAO (PETSc) 
# to eliminate the smoothing effects of tanh transformation and reproduce sharp slip patterns.
# 
# Key advantages over scipy version:
# * Native PETSc/hipPYlib integration (no array conversions)
# * TAO TAOBLMVM solver specifically designed for box constraints
# * More efficient memory usage and parallel scaling
# * Direct constraint enforcement like MATLAB lsqlin
# 
...
# %%
# TAO BOX-CONSTRAINED PDE Variational Formulation (NO TANH TRANSFORMATION)
...
# %%
# TAO Objective Function Class for hipPYlib Model
class TAOObjectiveFunction:
    """
    TAO-compatible objective function wrapper for hipPYlib models
    """
    ...
    def formObjective(self, tao, m_petsc):
        ...
        try:
            # Copy PETSc Vec data to hipPYlib vector
    ...
    def formGradient(self, tao, m_petsc, g_petsc):
        ...
        try:
            ...
            # Compute gradient
            grad_vec = self.model.generate_vector(hp.PARAMETER)
            self.model.evalGradientParameter(x, grad_vec)
            
            # Copy gradient to PETSc vector
            g_petsc.setArray(grad_vec.get_local())
...
# %%
# TAO Box-Constrained Inversion Function
def solveCoseismicInversion_TAO(k, targets, m0_s_expr, mtrue_mu_expr_inv, gamma_val_H1, delta_val_L2, \
                            box_transformer, savefiles=True, verbose=True):
    """
    TAO box-constrained slip inversion using PETSc TAO TAOBLMVM
    ...
    if verbose:
        print(sep, "TAO BOX-CONSTRAINED: Set up mesh and direct constraint framework", sep)
        print("Number of dofs: STATE={0}, PARAMETER={1}, ADJOINT={2}".format(*ndofs))
        print("Number of STATE and ADJOINT dofs: STRESS={0}, DISPLACEMENT={1}, ROTATION={2}".format(*ndofs_state))
        print("")
        print("TAO BOX CONSTRAINT FRAMEWORK")
    ...
    if verbose:
        print("Using TAO box-constrained PDE (no slip transformation)")
    ...
    # TAO SOLVER SETUP
    if verbose:
        print(sep, "Setting up TAO box-constrained solver", sep)
    ...
    if box_transformer.has_any_bounds:
        ...
        lb_petsc.set(-PETSc.INFINITY)
        ub_petsc.set(PETSc.INFINITY)
    ...
    objective = TAOObjectiveFunction(model, Vh)
    
    # Set TAO objective and gradient separately
    ...
    # Set initial solution (this is done in tao.solve())
    # No setInitialVector method - initial guess passed to solve()
    
    if verbose:
        print("TAO solver configured successfully")
    ...
    if verbose:
        print(f"TAO optimization completed in {optimization_time:.2f} seconds")
...
for gamma_s in gammas_s:
    ...
    results = solveCoseismicInversion_TAO(
