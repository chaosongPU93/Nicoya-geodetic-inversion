...
# 
#     * chile.xml: Contains the mesh data.
#     * chile_facet_region.xml: Contains boundary tags.
#     * chile_physical_region.xml: Contains subdomain tags.    
# 
...
obs_disp_name = "Feng_etal_JGR_2012table1.csv"
...
data = pd.read_csv(datadir + obs_disp_name, sep=",", skiprows=1, \
                   names=['site', 'lat', 'lon', 'height', 'Dt', 'Days', \
                          'uy_ITRF05', 'uy_std_ITRF05', 'ux_ITRF05', 'ux_std_ITRF05', 'uz_ITRF05', 'uz_std_ITRF05', \
                          'uy_Car', 'uy_std_Car', 'ux_Car', 'ux_std_Car', 'uz_Car', 'uz_std_Car'])
cols_to_convert = ['uy_ITRF05', 'uy_std_ITRF05', 'ux_ITRF05', 'ux_std_ITRF05', 'uz_ITRF05', 'uz_std_ITRF05', \
                   'uy_Car', 'uy_std_Car', 'ux_Car', 'ux_std_Car', 'uz_Car', 'uz_std_Car']
data[cols_to_convert] = data[cols_to_convert] / 1e3  # Convert mm to m

cols_to_reverse = ['uy_ITRF05', 'ux_ITRF05', 'uz_ITRF05', 'uy_Car', 'ux_Car', 'uz_Car']
data[cols_to_reverse] = -1*data[cols_to_reverse]    # reverse the sign to indicate 'back-slip'
...
def solveCoseismicInversion(k, targets, m0_s_expr, gamma_val_H1, delta_val_L2, savefiles=True, verbose=True):
    ...
    obs_weights[9::15]  = (1. / data['ux_std_Car']**2).to_numpy() * 1/(f_h**2)  # horizontal east displacement 
    obs_weights[10::15] = (1. / data['uy_std_Car']**2).to_numpy() * 1/(f_h**2)  # horizontal north displacement 
    obs_weights[11::15] = (1. / data['uz_std_Car']**2).to_numpy() * 1/(f_v**2)  # vertical displacements
    ...
    tmp[9::15] = np.array(data['ux_Car'])    # horizontal east displacement misfit
    tmp[10::15] = np.array(data['uy_Car'])   # horizontal north displacement misfit
    tmp[11::15] = np.array(data['uz_Car'])   # vertical displacement misfit
    ...
    solver.parameters["max_iter"]      = 2500
...
# file identifier
inv_str = f"_revlocking_w{w_h}{w_v}_gs{gamma_val_H1:.0e}_ds{delta_val_L2:.0e}"
...
outFileName = f"Lcurvers{rho_s:.0e}{meshname}.txt"
csvoutput = open(resultpath + outFileName, 'w+')
