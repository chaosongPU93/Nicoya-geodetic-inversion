# %% [markdown]
# # Back-slip inversion with direction constrained to plate motion azimuth
#
...
def solveCoseismicInversion_AzSlip(k, targets, m0_amp_expr, mtrue_mu_expr_inv,
    ...
    # Convert to Function for later use
    ...
    # Calculate seismic moment from physical slip (||slip|| = m_phys * ||c||)
    mtrue_mu_fun_expr = mu_expression(mtrue_mu_fun)
...
#           set RUN_LCURVE = True  to run full L-curves
# ============================================================
...
### set used for free 2-component inversion
# gammas_s = [4e2, 6e2, 8e2, 1e3, 2e3, 3e3, 4e3, 1e4, 5e4]

### gammas for unbounded inversion (linear mode)
# # Previously used at az=45 (kept for reference):
# gammas_s_hom = [2e2, 6e2, 3e3, 6e3]                                # extra
# gammas_s_het = [4e2, 8e2, 1e3, 2e3, 4e3, 1e4, 5e4, 2e2, 6e2, 3e3, 6e3]

# Two-point comparison at az=33.5° (N33.5E plate-convergence direction), bracketing
# the preferred damping picked off the az=45° L-curve corner.
gammas_s_hom = [4e3, 3e3]
gammas_s_het = [4e3, 3e3]

RUN_HOM_ONLY = False  # True = skip Het L-curve, run only Hom (append missing large-γ points)

# ============================================================
# SINGLE TEST RUN (RUN_LCURVE = False): Het μ, large damping
# ============================================================
if not RUN_LCURVE:
    ...
    print(sep, "SINGLE TEST RUN — Az-constrained scalar amplitude inversion", sep)
    ...
    results_test = solveCoseismicInversion_AzSlip(
        k, targets, m0_amp_expr, mtrue_mu_expr_het, gamma_s_test, delta_s_test,
        c_strike_fun, c_dip_fun, amp_max,
        use_tanh_amp=use_tanh_amp,
        savefiles=True, verbose=True)
...
# ============================================================
if RUN_LCURVE and not RUN_HOM_ONLY:
    # ============================================================
    # L-curve: Het μ model, Az-constrained amplitude
    # (HOM-ONLY RUN: this block is disabled)
    # ============================================================
    print(sep, "Het L-curve — Az-constrained scalar amplitude inversion", sep)
    print(f"  azimuth_deg = {azimuth_deg:.1f}°,  amp_max = {amp_max*1e3:.1f} mm/yr,  use_tanh_amp = {use_tanh_amp}")

    # L-curve file appended to incrementally so partial results survive crashes
    # and downstream plotting sees rows as they are computed.
    outFileName_lc = f"Lcurvelockbothaz{azimuth_deg:.0f}{amp_tag}_rs{rho_s:.0e}_{meshname}{mu_str_het}.txt"
    print("L-curve file:", outFileName_lc)

    dmisfit = []
    mmisfit = []
    for gamma_s in gammas_s_het:
    ...
    print("All Het Az Inversion Finished!")

# %%
# ============================================================
# L-curve: Hom μ model, Az-constrained amplitude (ACTIVE — appends missing large-γ points)
# ============================================================
if RUN_LCURVE:
    print(sep, "Hom L-curve — Az-constrained scalar amplitude inversion", sep)

    # L-curve file appended to incrementally so partial results survive crashes
    # and downstream plotting sees rows as they are computed.
    outFileName_lc = f"Lcurvelockbothaz{azimuth_deg:.0f}{amp_tag}_rs{rho_s:.0e}_{meshname}{mu_str_hom}.txt"
    print("L-curve file:", outFileName_lc)

    dmisfit_hom = []
    mmisfit_hom = []
    for gamma_s in gammas_s_hom:
        delta_s = gamma_s / rho_s
        inv_str = f"_lockbothaz{azimuth_deg:.0f}{amp_tag}_w{w_h}{w_v}_gs{gamma_s:.0e}_ds{delta_s:.0e}"
        print("Inverse problem identifier:", inv_str)
        print(f"****** gamma_s = {gamma_s:.1e}, delta_s = {delta_s:.1e}, rho_s = {rho_s:.1e} ******")
        ...
        misfit_d = float(results[-2])
        misfit_m = float(results[-1])
        dmisfit_hom.append(misfit_d)
        mmisfit_hom.append(misfit_m)
        del results
        gc.collect()
    ...
    print("All Hom Az Inversion Finished!")
