...
# %%
# --- Vertical slices helpers ---
def vertical_slice_along_strike(x_km, y_min=-150, y_max=150, z_min=0, z_max=-80, step=2.5):
    """x fixed (strike), grid in y-z."""
    y_vals = np.arange(y_min, y_max + step, step)
    z_vals = np.arange(z_min, z_max - step, -step)  # downward
    Y, Z = np.meshgrid(y_vals, z_vals)
    X = np.full_like(Y, fill_value=x_km)
    lon, lat = ut.ckm2LLd(X * 1e3 + x0, Y * 1e3 + y0, lon0, lat0, rot)
    return pd.DataFrame({"lon": lon.ravel(), "lat": lat.ravel(),
                         "x": X.ravel(), "y": Y.ravel(), "z": Z.ravel()})

def vertical_slice_along_dip(y_km, x_min=-150, x_max=150, z_min=0, z_max=-80, step=2.5):
    """y fixed (dip), grid in x-z."""
    x_vals = np.arange(x_min, x_max + step, step)
    z_vals = np.arange(z_min, z_max - step, -step)
    X, Z = np.meshgrid(x_vals, z_vals)
    Y = np.full_like(X, fill_value=y_km)
    lon, lat = ut.ckm2LLd(X * 1e3 + x0, Y * 1e3 + y0, lon0, lat0, rot)
    return pd.DataFrame({"lon": lon.ravel(), "lat": lat.ravel(),
                         "x": X.ravel(), "y": Y.ravel(), "z": Z.ravel()})

# Build multiple dip slices: y from -100 to 100 km every 10 km
y_vals = np.arange(-100, 100 + 10, 10)
dip_slice = [vertical_slice_along_dip(y_km=y) for y in y_vals]
vert_dip_slices = pd.concat(dip_slice, ignore_index=True)
print(f"Vertical dip slices: {len(y_vals)} slices, {len(vert_dip_slices)} points total")

# Build multiple strike slices: x from -140 to 60 km every 10 km
x_vals = np.arange(-140, 60 + 10, 10)
strike_slice = [vertical_slice_along_strike(x_km=x) for x in x_vals]
vert_strike_slices = pd.concat(strike_slice, ignore_index=True)
print(f"Vertical strike slices: {len(x_vals)} slices, {len(vert_strike_slices)} points total")
...
# meshname = "nicoyaCK4_1"   # same as CK2, but connecting the trench now
...
# %%
def computeGridDisplacements(k, targets, mtrue_mu_expr_for, gridname, mtrue_s_expr=None, pollute=False, \
                          pollute_type='uniform', savefiles=True, verbose=True):
    """
    ...
    if savefiles:
        ...
        outFileName = 'd_obs_grid' + meshname + slip_str_gt + mu_str_for + gridname + '.txt'
        csvoutput = open(resultpath + outFileName, 'w+')
        for i in range(0, targets.shape[0]):
            csvoutput.write( "%.6f %.6f %.6f %.6f %.6f %.6f\n" %(
...
# Dense grid slices 
def build_targets_grid(dataframe):
    ntargets_dense = len(dataframe)
    targets_grid = np.zeros([ntargets_dense, 3])
    targets_grid[:,0] = np.array(dataframe['x'])*1e3   # km to m
    targets_grid[:,1] = np.array(dataframe['y'])*1e3
    targets_grid[:,2] = np.array(dataframe['z'])*1e3
    print(targets_grid.shape)

    return targets_grid

def write_slices_coordinates(dataframe, resultpath, coord_file):
    coord_output = open(resultpath + coord_file, 'w+')
    for i in range(0, dataframe.shape[0]):
        coord_output.write("%.6f %.6f %.6f %.6f %.6f\n" %(
            dataframe['lon'].iloc[i], dataframe['lat'].iloc[i],
            dataframe['x'].iloc[i], dataframe['y'].iloc[i], dataframe['z'].iloc[i]
        ))
    coord_output.close()
...
targets_grid_hori = build_targets_grid(hori_slices)
gridname_h = "hori_slices"
coord_file = f"{gridname_h}_coordinates_{meshname}.txt"
write_slices_coordinates(hori_slices, resultpath, coord_file)

targets_grid_vert_s = build_targets_grid(vert_strike_slices)
gridname_vs = "vert_strike_slices"
coord_file = f"{gridname_vs}_coordinates_{meshname}.txt"
write_slices_coordinates(vert_strike_slices, resultpath, coord_file)

targets_grid_vert_d = build_targets_grid(vert_dip_slices)
gridname_vd = "vert_dip_slices"
coord_file = f"{gridname_vd}_coordinates_{meshname}.txt"
write_slices_coordinates(vert_dip_slices, resultpath, coord_file)
...
computeGridDisplacements(k, targets_grid_hori, mtrue_mu_expr_for, gridname_h, mtrue_s_expr=mtrue_s_expr_for, \
                      pollute=False, pollute_type=pollute_type, savefiles=True, verbose=True)
print("Dense horizontal grid slices forward problem done!!!")

computeGridDisplacements(k, targets_grid_vert_s, mtrue_mu_expr_for, gridname_vs, mtrue_s_expr=mtrue_s_expr_for, \
                      pollute=False, pollute_type=pollute_type, savefiles=True, verbose=True)
print("Dense vertical strike grid slices forward problem done!!!")

computeGridDisplacements(k, targets_grid_vert_d, mtrue_mu_expr_for, gridname_vd, mtrue_s_expr=mtrue_s_expr_for, \
                      pollute=False, pollute_type=pollute_type, savefiles=True, verbose=True)
print("Dense vertical dip grid slices forward problem done!!!")
