# %% [markdown]
# # Back-slip inversion with OPTIMIZED TAO BOX CONSTRAINTS (PETSc native)
# 
# This OPTIMIZED version implements efficient box-constrained optimization using TAO (PETSc) 
# with combined objective/gradient evaluation to minimize forward/adjoint solve overhead.
# 
# Key optimizations over standard TAO version:
# * Combined objective+gradient evaluation (single forward/adjoint solve)
# * Cached state vectors to avoid redundant computations
# * Efficient PETSc vector operations
# * Optimized callback interface
# 
# Based on: slip_inv_hetmu_nicoyaCK_locking_both_tao.py
...
# %%
# OPTIMIZED TAO BOX-CONSTRAINED PDE Variational Formulation (NO TANH TRANSFORMATION)
class PDEVarf_TAOBoxConstrained:
    """
    TAO box-constrained PDE variational formulation
    
    Key difference: m parameters are PHYSICAL SLIP directly (no transformation)
    Bounds are enforced by TAO solver, not by parameterization
    """
...
# %%
# OPTIMIZED TAO Objective Function Class for hipPYlib Model
class OptimizedTAOObjectiveFunction:
    """
    OPTIMIZED TAO-compatible objective function wrapper for hipPYlib models
    
    Key optimizations:
    - Combined objective+gradient evaluation (single forward/adjoint solve)
    - Cached vectors to avoid repeated allocations
    - Efficient parameter update mechanism
    """
    ...
    def _compute_param_hash(self, m_array):
        """Simple hash to detect if parameters changed"""
        return hash(m_array.tobytes())
    
    def _update_parameter_vector(self, m_petsc):
        """Efficiently update hipPYlib parameter vector from PETSc vector"""
        m_array = m_petsc.getArray()
        self.mg.set_local(m_array.copy())
        self.mg.apply('')
        return m_array
        
    def _solve_forward_adjoint(self, x):
        """Solve both forward and adjoint problems efficiently"""
    ...
    def _compute_if_needed(self, m_petsc):
        """Compute objective and gradient if parameters changed"""
        ...
        if current_hash == self.last_m_hash and self.last_cost is not None:
            self.cache_hits += 1
        ...
        self.last_m_hash = current_hash
        self.last_cost = cost
        self.last_gradient = gradient_array.copy()
        
        # Optional: Print progress every N evaluations
        if self.eval_count % 10 == 0:
            print(f"TAO eval {self.eval_count}: cost = {cost:.6e}, cache hits = {self.cache_hits}")
    
    def formObjective(self, tao, m_petsc):
        """TAO callback for objective evaluation"""
        try:
    ...
    def formGradient(self, tao, m_petsc, g_petsc):
        """TAO callback for gradient evaluation"""
        try:
            self._compute_if_needed(m_petsc)
...
# %%
# OPTIMIZED TAO Box-Constrained Inversion Function
def solveCoseismicInversion_TAOOptimized(k, targets, m0_s_expr, mtrue_mu_expr_inv, gamma_val_H1, delta_val_L2, \
                            box_transformer, savefiles=True, verbose=True):
    """
    OPTIMIZED TAO box-constrained slip inversion using PETSc TAO TAOBLMVM
    
    Key optimizations:
    - Combined objective+gradient evaluation
    ...
    if verbose:
        print(sep, "OPTIMIZED TAO BOX-CONSTRAINED: Set up mesh and direct constraint framework", sep)
        print("Number of dofs: STATE={0}, PARAMETER={1}, ADJOINT={2}".format(*ndofs))
        print("Number of STATE and ADJOINT dofs: STRESS={0}, DISPLACEMENT={1}, ROTATION={2}".format(*ndofs_state))
        print("")
        print("OPTIMIZED TAO BOX CONSTRAINT FRAMEWORK")
        print(f"   {box_transformer}")
        print("   Optimizations: Combined obj/grad, cached vectors, reduced allocations")
    ...
    # Use TAO box-constrained PDE variational formulation
    pde_varf = PDEVarf_TAOBoxConstrained(mtrue_mu_fun)
    if verbose:
        print("Using optimized TAO box-constrained PDE (no slip transformation)")
    ...
    # OPTIMIZED TAO SOLVER SETUP
    if verbose:
        print(sep, "Setting up OPTIMIZED TAO box-constrained solver", sep)
    ...
    objective = OptimizedTAOObjectiveFunction(model, Vh)
    ...
    # Set TAO parameters
    tao.setTolerances(gatol=1e-9, grtol=1e-9, gttol=1e-12)
    tao.setMaximumIterations(1000)
    
    # Set TAO options from command line (if any)
    tao.setFromOptions()
    
    if verbose:
        print("OPTIMIZED TAO solver configured successfully")
        print("Features: Combined obj/grad, caching, performance monitoring")
        print("Starting optimization...")
    
    # SOLVE with OPTIMIZED TAO
    ...
    if verbose:
        print(f"OPTIMIZED TAO optimization completed in {optimization_time:.2f} seconds")
        ...
        print(f"Cache hit rate: {100*objective.cache_hits/objective.eval_count:.1f}%")
...
for gamma_s in gammas_s:
    ...
    results = solveCoseismicInversion_TAOOptimized(
