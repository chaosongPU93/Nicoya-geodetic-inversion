...
# df['lon'], df['lat'] = ut.ckm2LLd(df['x'], df['y'], lon0, lat0, -rot)
# print(df.head())

# import matplotlib.pyplot as plt

# # abs loc between used mesh, and stations
...
# plt.scatter(lon0, lat0, s=40, color='black', marker='s')  # solid dots
...
# # relative loc between used mesh, and stations, after offseting 
# plt.figure(figsize=(6, 6))
# plt.scatter((df['x'] - x0)/1e3, (df['y'] - y0)/1e3, s=10, color='blue', marker='o')  # solid dots
# plt.scatter(data['x'], data['y'], s=10, color='red', marker='^')  # solid dots
# plt.xlabel("Along-strike (x) [km]")
# plt.ylabel("Along-dip (y) [km]")
# plt.axis("equal")  # equal scaling for x/y
# plt.grid(True)
# plt.show()
...
# m0_s_expr = dl.Constant((-2., -2.))
# m0_s_expr = dl.Constant((0., -0.1))
print( "Starting model for slip: ", m0_s_expr.values() )
...
if BOUNDED:
    ...
    if BOUND_TYPE == 'both':
        slip_transformer = SlipTransformation(
            ...
            dip_bounds=(0.0-10e-3, s_dip_max),
        )
...
if not BOUNDED:
    ...
    results = solveCoseismicInversion_TanhSlip(
        k, targets, m0_s_expr, mtrue_mu_expr_inv, gamma_val_H1, delta_val_L2,
        slip_transformer,  # Bounded transformer
        savefiles=True, verbose=True
    )

else:    
    ### two-step solution
    # ===== STEP 1: UNBOUNDED WARM-START INVERSION =====
    # ... (solve unbounded problem to get physical slip solution)
    inv_str = f"_lockboth_w{w_h}{w_v}_gs{gamma_val_H1:.0e}_ds{delta_val_L2:.0e}"
    print("Inverse problem identifier: ", inv_str)
    print("Solving inverse problem based on: ", mu_str_inv)
    results_warmstart = solveCoseismicInversion_TanhSlip(
            k, targets, m0_s_expr, mtrue_mu_expr_inv, gamma_val_H1, delta_val_L2,
            SlipTransformation(strike_bounds=None, dip_bounds=None),
            savefiles=True, verbose=True
        )
    m_physical_slip = results_warmstart[4]  # This is physical slip

    # ===== STEP 2: CONVERT PHYSICAL SLIP TO UNBOUNDED PARAMETERS =====
    # Apply inverse transformation: physical_slip → unbounded_parameters
    # Vm = dl.VectorFunctionSpace(mesh, "CG", degree=1, dim=2)
    # m_physical_fun = hp.vector2Function(m_physical_slip, Vm)

    # Split physical slip into components
    m_physical_array = m_physical_slip.get_local()
    m_strike_phys = m_physical_array[0::2]  # Strike component
    m_dip_phys = m_physical_array[1::2]     # Dip component

    # Apply inverse tanh transformation
    # For strike: m_unbounded = atanh(2 * (s_phys - s_min) / (s_max - s_min) - 1)
    if slip_transformer.has_strike_bounds:
        s_normalized = 2 * (m_strike_phys - slip_transformer.strike_min) / \
                        (slip_transformer.strike_max - slip_transformer.strike_min) - 1
        # Clamp to avoid atanh(±1) = ±∞
        s_normalized = np.clip(s_normalized, -0.999, 0.999)
        m_strike_unbounded = np.arctanh(s_normalized)
    else:
        m_strike_unbounded = m_strike_phys

    if slip_transformer.has_dip_bounds:
        s_normalized = 2 * (m_dip_phys - slip_transformer.dip_min) / \
                        (slip_transformer.dip_max - slip_transformer.dip_min) - 1
        s_normalized = np.clip(s_normalized, -0.999, 0.999)
        m_dip_unbounded = np.arctanh(s_normalized)
    else:
        m_dip_unbounded = m_dip_phys

    # Reconstruct unbounded parameter vector
    m_unbounded_array = np.zeros_like(m_physical_array)
    m_unbounded_array[0::2] = m_strike_unbounded
    m_unbounded_array[1::2] = m_dip_unbounded

    # Create new vector and set values (corrected approach)
    m_unbounded = m_physical_slip.copy()  # Copy structure
    m_unbounded.set_local(m_unbounded_array)
    m_unbounded.apply('')  # Apply parallel assembly

    # Convert to Function for starting point
    Vm = dl.VectorFunctionSpace(mesh, "CG", degree=1, dim=2)
    m_unbounded_fun = hp.vector2Function(m_unbounded, Vm)

    print(f"Physical slip range: [{m_physical_array.min():.6f}, {m_physical_array.max():.6f}]")
    print(f"Unbounded params range: [{m_unbounded_array.min():.6f}, {m_unbounded_array.max():.6f}]")

    # ===== STEP 3: BOUNDED INVERSION WITH CORRECT STARTING POINT =====
    # use different regularization weights
    gamma_val_H1 = 5e1  
    delta_val_L2 = 5e-7
    # file identifier
    ...
    print("Inverse problem identifier: ", inv_str)
    results = solveCoseismicInversion_TanhSlip(
        k, targets, m_unbounded_fun, mtrue_mu_expr_inv, gamma_val_H1, delta_val_L2,
        slip_transformer,  # Bounded transformer
        savefiles=True, verbose=True
    )
