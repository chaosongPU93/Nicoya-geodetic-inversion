...
print("Loading slip transformation framework from utility...")
...
if amp < 0.1:  # SSE case (your V_norm = 0.0785 m)
    slip_transformer = SlipTransformation(
        strike_bounds=(-0.5 * amp, 0.5 * amp),    # Limited strike-slip (±39 mm)
        dip_bounds=(0.0, 2.0 * amp)               # Thrust only, up to 2x expected (157 mm)
    )
    print("✅ SSE-style slip bounds configured")
elif amp >= 1.0:  # Large earthquake
    slip_transformer = SlipTransformation(
        strike_bounds=(-10.0, 10.0),              # ±10 m
        dip_bounds=(0.0, 50.0)                    # 0-50 m thrust
    )
    print("✅ Large earthquake slip bounds configured")  
else:  # Moderate case
    slip_transformer = SlipTransformation(
        strike_bounds=(-1.0, 1.0),                # ±1 m
        dip_bounds=(0.0, 5.0)                     # 0-5 m thrust
    )
    print("✅ Moderate earthquake slip bounds configured")
