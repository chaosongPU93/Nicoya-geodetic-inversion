...
from slip_transformation_utils_enhanced import SlipTransformation
...
# %%
# ENHANCED: PDE Variational Formulation with Configurable Slip Transformation
class PDEVarf_EnhancedSlip:
    """
    Enhanced PDE variational formulation with configurable slip transformation functions
    
    Key differences from original:
    - m (unbounded parameters) → physical_slip (bounded) via slip_transformer
    - Supports multiple transformation functions: tanh, arctan, sigmoid, sqrt
    - Uses slip_transformer's built-in transformation methods
    ...
    def __call__(self, u, m, p):
        ...
        # 🎯 KEY ENHANCEMENT: Transform individual components using selected function
        # Apply transformations component-wise to avoid UFL splitting issues
        m_strike_phys, m_dip_phys = self.slip_transformer.transform_component_wise(m_strike, m_dip)
...
def solveCoseismicInversion_TransSlip(k, targets, m0_s_expr, mtrue_mu_expr_inv, gamma_val_H1, delta_val_L2,
    ...
    if verbose:
        ...
        if slip_transformer.has_any_bounds:
            ...
            print("     m ∈ (-∞,∞) → physical_slip ∈ [bounds] via", FUNCTION_TYPE, "transformation")
        else:
    ...
    if slip_transformer.has_any_bounds:
        pde_varf = PDEVarf_EnhancedSlip(mtrue_mu_fun, slip_transformer)
        if verbose:
            print("Using enhanced PDE with", FUNCTION_TYPE, "slip transformation")
    else:
        pde_varf = PDEVarf(mtrue_mu_fun)
        if verbose:
    ...
    if verbose:
        ...
        print("(slip bounds enforced automatically via", FUNCTION_TYPE, "transformation)")
...
print("Solving inverse problem based on: ", mu_str_inv)
results = solveCoseismicInversion_TransSlip(
        k, targets, m0_s_expr, mtrue_mu_expr_inv, gamma_val_H1, delta_val_L2,
        slip_transformer,  # KEY: Pass slip transformation
        pollute=pollute, pollute_type=pollute_type, savefiles=True, verbose=True
    )
...
# print( sep, "L-curve computation with slip transformation", sep )
...
#         pollute=pollute, pollute_type=pollute_type, savefiles=True, verbose=True
#     )
#     print("Hom Inversion with slip bounds finished!!!")


# print("All Inversion with Slip Bounds Finished!")   