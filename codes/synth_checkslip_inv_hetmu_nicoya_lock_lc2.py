...
# 
# * This version do NOT add any noise to the synthetic data, define ``misfit`` without ``weight`` option, and set the ``misfit.noise_variance`` to 1 during the inversion. 
#
# * L-curve type, meant to run a few different gamma values with the same gamma/delta ratio=1e8, for 20 km slip patch size
...
def solveCoseismicForward(k, targets, mtrue_mu_expr_for, mtrue_s_expr=None, pollute=True, savefiles=True, verbose=True):
    ...
    if pollute:
        ...
        hp.Random().normal_perturb(noise_std_h, misfit.d) # Comment if no data noise
...
def solveCoseismicInversion(k, targets, m0_s_expr, mtrue_mu_expr_inv, gamma_val_H1, delta_val_L2, savefiles=True, verbose=True):
...
# slip_str_gt = f"_gauss_x{x0:g}_y{x0:g}_s{sigma/1e3:g}_rs{maxamp:g}"
# print(slip_str_gt)
...
# slip_str_gt = f"_const_x{x0/1e3:g}_y{y0/1e3:g}_r{radius/1e3:g}_ms{maxamp:g}"
# print(slip_str_gt)
...
# # noise_std_h = 0.5 * (data['sig_ux'].median() + data['sig_uy'].median())
...
# solveCoseismicForward(k, targets, mtrue_mu_expr_for, mtrue_s_expr=mtrue_s_expr_for, pollute=True, savefiles=True, verbose=True)
solveCoseismicForward(k, targets, mtrue_mu_expr_for, mtrue_s_expr=mtrue_s_expr_for, pollute=False, savefiles=True, verbose=True)
...
# solveCoseismicInversion(k, targets, m0_s_expr, mtrue_mu_expr_inv, gamma_val_H1, delta_val_L2, savefiles=True, verbose=True)
...
# rho_s = 1e9
rho_s = 1e8 # for a correlation length of 10 km
...
gammas_s = [1e1, 4e1, 1e2, 2e2, 4e2, 8e2, 1e3]
...
for gamma_s in gammas_s:
    ...
    mu_str_inv = mu_str_het
    print("Solving inverse problem based on: ", mu_str_inv)
    solveCoseismicInversion(k, targets, m0_s_expr, mtrue_mu_expr_inv, gamma_s, delta_s, savefiles=True, verbose=True)
    print("Het Inversion finished!!!")
