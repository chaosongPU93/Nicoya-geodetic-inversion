...
def solveCoseismicForward(k, targets, mtrue_mu_expr_for, mtrue_s_expr=None, pollute=True, \
    ...
    if verbose:
        print("=" * 80)
        print("WARNING: Using ITERATIVE solver (GMRES) instead of MUMPS direct solver")
        print("This is a workaround for 32-bit PETSc integer limitations")
        print("=" * 80)

    # Create iterative Krylov solver with preconditioner
    ...
    # Enable convergence monitoring to diagnose potential solver issues
    pde.solver.parameters["monitor_convergence"] = True if verbose else False

    pde.solver_fwd_inc = hp.PETScKrylovSolver(mesh.mpi_comm(), "gmres", "hypre_amg")
    pde.solver_fwd_inc.parameters["relative_tolerance"] = 1e-9
    pde.solver_fwd_inc.parameters["absolute_tolerance"] = 1e-12
    pde.solver_fwd_inc.parameters["maximum_iterations"] = 5000
    pde.solver_fwd_inc.parameters["monitor_convergence"] = True if verbose else False

    pde.solver_adj_inc = hp.PETScKrylovSolver(mesh.mpi_comm(), "gmres", "hypre_amg")
    pde.solver_adj_inc.parameters["relative_tolerance"] = 1e-9
    pde.solver_adj_inc.parameters["absolute_tolerance"] = 1e-12
    pde.solver_adj_inc.parameters["maximum_iterations"] = 5000
    pde.solver_adj_inc.parameters["monitor_convergence"] = True if verbose else False
