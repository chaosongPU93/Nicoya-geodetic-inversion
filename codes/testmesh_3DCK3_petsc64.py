# %% [markdown]
# # Synthetic Slip inversion - 64-BIT PETSC VERSION
#
# **IMPORTANT: This version requires PETSc with 64-bit integer support**
#
# To run this script:
# 1. Activate the fenics64 environment: `conda activate fenics64`
# 2. Set environment variables:
#    export PETSC_DIR=~/software/petsc_64bit/petsc-3.20.1
#    export PETSC_ARCH=linux-gnu-c-opt-64bit
# 3. Run: python testmesh_3DCK3_petsc64.py
#
# This version uses MUMPS direct solver and can handle large meshes (dense4, dense5, dense6)
# without the 32-bit integer overflow that causes segmentation faults.
#
# ---
#
...
# %%
# VERIFY 64-BIT PETSC INTEGER SUPPORT
print("\n" + "="*80)
print("VERIFYING 64-BIT PETSC CONFIGURATION")
print("="*80)
try:
    ...
    print(f"PETSc Integer Type: {petsc_int_type}")
    print(f"PETSc Version: {PETSc.Sys.getVersion()}")

    if petsc_int_type == np.int64:
        print("✓ SUCCESS: Using 64-bit PETSc integers")
        print("  This script can handle large meshes (dense4, dense5, dense6)")
    else:
        print("✗ ERROR: Using 32-bit PETSc integers")
        print("  This script requires 64-bit PETSc to avoid segmentation faults")
        print("  Please activate fenics64 environment and set PETSC_DIR/PETSC_ARCH")
        print("\nTo fix:")
        print("  conda activate fenics64")
        print("  export PETSC_DIR=~/software/petsc_64bit/petsc-3.20.1")
        print("  export PETSC_ARCH=linux-gnu-c-opt-64bit")
        raise RuntimeError("32-bit PETSc detected! Need 64-bit PETSc for this script.")

except ImportError as e:
    print(f"✗ ERROR: Cannot import petsc4py: {e}")
    raise
...
def solveCoseismicForward(k, targets, mtrue_mu_expr_for, mtrue_s_expr=None, pollute=True, \
    ...
    # Use MUMPS direct solver (requires 64-bit PETSc for large meshes)
    if verbose:
        print("=" * 80)
        print("Using MUMPS direct solver with 64-bit PETSc")
        print("This version can handle large meshes (dense4, dense5, dense6)")
        print("=" * 80)
    ...
    # Diagnostic: Check if forward solve produced non-zero solution
    if verbose:
        state_norm = x[hp.STATE].norm('l2')
        print(f"Forward solve completed. State vector L2 norm: {state_norm:.6e}")
        if state_norm < 1e-15:
            print("WARNING: Forward solve produced near-zero solution! Check solver convergence.")
    ...
    # Diagnostic: Check if observation operator produced non-zero data
    if verbose:
        d_norm = np.linalg.norm(misfit.d.get_local())
        n_nonzero = np.count_nonzero(misfit.d.get_local())
        print(f"Observation data norm: {d_norm:.6e}, non-zero entries: {n_nonzero}/{len(misfit.d)}")
        if d_norm < 1e-15:
            print("WARNING: Observation operator produced near-zero data!")
    ...
    # Fix: Determine idx_d based on structure, not based on whether misfit.d is non-zero
    # Since we have targets.shape[0] observation points with 3 displacement components each
    idx_d = list(range(3 * targets.shape[0]))
