# %% [markdown]
# # Synthetic forward (Az-consistent) + scalar amplitude inversion — STRIPE, 3D DeShon μ
#   nicoyaden_sm mesh (Slab2.0 plate interface), azi_equidist_proj coordinates.
#
# Based on: synth_stripe_az_consistfwd_nicoya.py    (Hom CF strategy, nicoyaden_sm)
#           synth_stripe_az_consistfwd_3D_nicoyaCK.py  (3D μ machinery, CK mesh version)
#
# PURPOSE: Run the AzSlip-consistent forward (3D DeShon μ, CG2) + scalar amplitude inversion
#          on the nicoyaden_sm mesh. Inversion tested under both 3D Het and Hom μ (2-pass L-curve).
#
...
#              mu = dl.Function (CG2) from process_velocity_models_hull + scale_shear_modulus_by_1d
#              SAME operator as solveCoseismicInversion_AzSlip → no mismatch
#   Inversion: SCALAR amplitude a(x) ∈ [0, amp_max] via tanh; direction fixed to Az
#
# COORDINATE SYSTEM: azi_equidist_proj  (lon0=-85.5+360, lat0=10, no rotation)
#   vel3d lon/lat also converted via azi_equidist_proj (not LL2ckmd).
#
# OUTPUTS (tagged with _cf_):
#   caz_diag_<meshname>_az<deg>.txt         per-vertex: xf yf zf c_s c_d c_mag
#   slip_true_cf_<meshname>...txt           per-vertex: xf yf zf m_phys s_strike s_dip slip_mag c_mag
#   d_obs_cf_<meshname>...txt               station displacements (consistent forward)
#   d_obs_grid_cf_<meshname>...txt          dense grid displacements
#   mu_true_..._dofs.h5                     CG2 DOF coordinates + values
#   Lcurvesynscaamp_cf_azbd_...txt          L-curve misfit vs model norm
...
sep = "\n"+"#"*80+"\n"
print(sep, "START of Az-consistent forward (stripe, 3D DeShon mu, nicoyaden_sm)", sep)
...
# nicoyaden_sm coordinate system: azi_equidist_proj (no rotation, no offset subtraction)
lon0, lat0 = -85.5+360, 10
data['x'], data['y'] = ut.azi_equidist_proj(data['lon'], data['lat'], lon0, lat0)
data['z'] = 0.0
# Note: azi_equidist_proj returns km; NO velocity rotation needed for this mesh
...
volc[['x', 'y']] = volc[['x', 'y']] * 1e3   # km → m for volc
volc['z'] = 0.0

# %%
# 3D velocity model (DeShon 2006) — coord transform via azi_equidist_proj (not LL2ckmd)
veldir = "/home/staff/chao/SSEinv/Nicoya/DeShon_2006GJI/"
vel3d = pd.read_csv(veldir + "DeShon2006_3Dmodel.csv", sep=",")
x_3d, y_3d = ut.azi_equidist_proj(vel3d['lon'], vel3d['lat'], lon0, lat0)
vel3d['x'] = x_3d * 1e3   # km → m
vel3d['y'] = y_3d * 1e3   # km → m
vel3d['z'] = vel3d['z'] * -1 * 1e3   # km depth (positive) → m (negative)
...
z_dense = np.zeros(n_2d)   # nicoyaden_sm has even top boundary at z=0
...
# %%
# ============================================================================
# HELPER: LEVEL 2 extrapolation for grid points outside mesh footprint
# ============================================================================
...
# %%
# Mesh selection — nicoyaden_sm: Slab2.0 plate interface, even top at z=0
meshname = "nicoyaden_sm"
# meshname = "nicoyaden_all"
print(meshname)
...
class PDEVarf_AzSlip_Direct:
    ...
    mu = self.mtrue_mu_fun directly — must already be in GPa.
    For 3D: pass dl.Function from process_velocity_models_hull (already GPa).
    For K_2LAYER/hom: pass mu_expression(param_fun) UFL expr (already GPa).
    """
    ...
    def __call__(self, u, m, p):
        ...
        mu = self.mtrue_mu_fun  # already in GPa
...
def build_scalar_amp_from_vec_expr(mtrue_s_expr, boundaries, fault, mesh):
    ...
    v2d_vec = dl.vertex_to_dof_map(V_slip_2)
    v2d_sca = dl.vertex_to_dof_map(Vm_amp)
...
def solveCoseismicForward_AzConsistent(k, targets, mtrue_mu_expr_for,
    ...
    mtrue_mu_expr_for = None  → 3D velocity model (CG2)
    mtrue_mu_expr_for = expr  → K_2LAYER/homogeneous (CG1)
    """
    ...
    if mtrue_mu_expr_for is None:
        ...
        mtrue_mu_param_fun = hp.vector2Function(mtrue_mu, CG_mu)
        mtrue_mu_fun = mu_expression(mtrue_mu_param_fun)  # UFL expr in GPa
    ...
    if savefiles:
        ...
        save_function_with_dofs(m_mu_true_save,
                                resultpath + 'mu_true_' + meshname + mu_str_for,
    ...
    slip_mag_fault  = m_phys_fault * c_mag_fault
    ...
    if isinstance(mtrue_mu_fun, dl.Function):
        mu_for_moment = mtrue_mu_fun
    else:
        mu_for_moment = dl.Function(CG_mu)
        mu_for_moment.vector()[:] = 20.0 * (2.0 + np.tanh(mtrue_mu_param_fun.vector()[:]))
    moment_true = dl.assemble(mu_for_moment * GPa2Pa * amp_fun_true * c_mag_fun * dS(fault))
    ...
    return mtrue_mu_fun, u_fwd, xf, yf, zf, d_obs, m_phys_fault, s_strike_fault, s_dip_fault
...
class PDEVarf_AzSlip:
    ...
    mu = self.mtrue_mu_fun directly — already in GPa (Function for 3D, UFL expr for K_2LAYER).
    """
    ...
    def __call__(self, u, m, p):
        ...
        mu = self.mtrue_mu_fun  # already in GPa (Function for 3D, UFL expr for K_2LAYER)
        J = ufl.inner(AEsigma(sigma, mu, nu), tau)*ufl.dx \
          + ufl.inner(ufl.div(tau), uu)*ufl.dx \
          + ufl.inner(asym(tau), r)*ufl.dx \
          + ufl.inner(ufl.div(sigma), w)*ufl.dx \
...
def solveCoseismicInversion_AzSlip(k, targets, m0_amp_expr, mtrue_mu_expr_inv,
    ...
    """
    Az-constrained scalar amplitude inversion (consistent-forward version, 3D support).

    mtrue_mu_expr_inv = None  → 3D velocity model (process_velocity_models_hull, CG2)
    mtrue_mu_expr_inv = expr  → K_2LAYER or homogeneous (CG1)

    Reads synthetic data from global `syndata` (loaded from d_obs_cf_* file).
    """
    BDM = dl.VectorFunctionSpace(mesh, "BDM", k)
    DGv = dl.VectorFunctionSpace(mesh, "DG", k-1)
    DGr = dl.VectorFunctionSpace(mesh, "DG", k-1)
    ...
    if mtrue_mu_expr_inv is None:
        ...
        # K_2LAYER or homogeneous: tanh-parameterized
    ...
    if savefiles:
        ...
        if isinstance(mtrue_mu_fun, dl.Function):
            ...
            m_mu_true_save.vector()[:] = 20.0 * (2.0 + np.tanh(m_values))
        save_function_with_dofs(m_mu_true_save,
                                resultpath + 'mu_true_' + meshname + mu_str_inv,
                                'shear modulus', cg_degree=CG_mu_deg)
    ...
    if isinstance(mtrue_mu_fun, dl.Function):
        m_mu_true_inv = mtrue_mu_fun
    else:
        m_mu_true_inv = dl.Function(CG_mu)
        m_values = mtrue_mu_param_fun.vector()[:]
        m_mu_true_inv.vector()[:] = 20.0 * (2.0 + np.tanh(m_values))
    moment_inv = dl.assemble(m_mu_true_inv * GPa2Pa * m_phys_fun * c_mag_fun_inv * dS(fault))
...
# 3D heterogeneous model — uses DeShon velocity model inside solvers
...
# %%
# Az configuration — nicoyaden_sm: mesh is not rotated → mesh_rotation_deg = 0.0
azimuth_deg       = 33.5   # CW from North; N33.5E
mesh_rotation_deg = 0.0    # nicoyaden_sm is not rotated (unlike nicoyaCKden which uses rot=45)
...
if slip_pattern == "checker":
    ...
    if pattern_option == 1:
        # nicoyaden_sm parameters: x_len=90 km, y0=-40 km (different from nicoyaCKden x_len=80, y0=-45)
...
if mtrue_mu_expr_for is None:
    ...
    CG_mu_deg = 1
print(f"Forward problem based on: {mu_str_for}, CG_mu_deg = {CG_mu_deg}")
...
# %%
# ============================================================================
# BUILD SCALAR AMPLITUDE FUNCTION FROM STRIPE EXPRESSION
# ============================================================================
print(sep, "Building scalar amplitude function from 2-component stripe expression", sep)
...
results_fwd = solveCoseismicForward_AzConsistent(
    ...
    savefiles=True, verbose=True, CG_mu_deg=CG_mu_deg)
mtrue_mu_fun_fwd, u_fwd, xf, yf, zf, d_obs, m_phys_fault, s_strike_fault, s_dip_fault = results_fwd
...
if not RUN_LCURVE:
    ...
    print(sep, "SINGLE TEST RUN — Az scalar amplitude inversion (consistent forward, 3D)", sep)
    ...
    # Hom inversion (mismatched: forward=3D, inversion=hom, CG1)
    mu_str_inv = mu_str_hom
    inv_str = f"_cf_synscaamp_azbd_w{w_h}{w_v}_gs{gamma_s:.1e}_ds{delta_s:.1e}"
    print(sep, "SINGLE TEST RUN — hom inversion (mismatched model, CG1)", sep)
    results = solveCoseismicInversion_AzSlip(
...
# %%
# ============================================================
# L-CURVE  (INACTIVE — set RUN_LCURVE = True to activate)
# ============================================================
if RUN_LCURVE:
    # gammas_s_priority = [1e2, 1.2e2, 1.5e2, 1.8e2, 2e2]
    gammas_s_priority = [1.5e2, 1.8e2]

    outFileName_het = (f"Lcurvesynscaamp_cf_azbd_rs{rho_s:.0e}_{meshname}"
                       f"_{slip_pattern}_{pattern_option}_{mu_str_for}"
                       f"_{mu_str_het}_CG{CG_mu_deg}.txt")
    ...
    # Order: Het 3D priority → Hom priority
    _passes = [
        ("Het 3D L-curve (priority)", gammas_s_priority, outFileName_het, mtrue_mu_expr_het,
         mu_str_het + f"_CG{CG_mu_deg}", CG_mu_deg),
        ("Hom L-curve (priority)",    gammas_s_priority, outFileName_hom, mtrue_mu_expr_hom,
         mu_str_hom, 1),
    ]
