# %% [markdown]
# # Slip inversion of the GNSS coseismic displacement measurements of 2012 Nicoya (Costa Rica) Mw7.6 EQ within a prescribed heterogeneous half-space
# 
# * Data (station location info and disp. observations) is stored under "Nicoya/data/Protti_etal_2014_tableS1.csv"
...
def solveCoseismicInversion_TanhSlip(k, targets, m0_s_expr, mtrue_mu_expr_inv, gamma_val_H1, delta_val_L2, 
    ...
    # # Recombine into mixed Function (strike+dip together)
    # W = dl.FunctionSpace(mesh, dl.MixedElement([s_strike_fun.function_space().ufl_element(),
    #                                             s_dip_fun.function_space().ufl_element()]))
    # slip_fun = dl.Function(W)
    # dl.assign(slip_fun.sub(0), s_strike_fun)
    # dl.assign(slip_fun.sub(1), s_dip_fun)

    # if savefiles:
    #     print("Saving slip solution to one .xdmf file")
    #     slip_fun.rename("coseismic slip", "coseismic slip")
    #     with dl.XDMFFile(resultpath + f"slip_{meshname}{inv_str}{mu_str_inv}.xdmf") as s_id:
    #         s_id.write(slip_fun)
    #     print("Finish saving slip solution")    
...
# file identifier
if BOUNDED:
    inv_str = f"_coseisbd_w{w_h}{w_v}_gs{gamma_val_H1:.0e}_ds{delta_val_L2:.0e}"
else:
...
### Find the best regularization parameters via homogeneous inversions and adapt for other structures ###
...
for gamma_s in gammas_s:
    ...
    # file identifier
    if BOUNDED:
        inv_str = f"_coseisbd_w{w_h}{w_v}_gs{gamma_s:.0e}_ds{delta_s:.0e}"
    else:
        inv_str = f"_coseis_w{w_h}{w_v}_gs{gamma_s:.0e}_ds{delta_s:.0e}"
    print("Inverse problem identifier: ", inv_str)
    ...
    misfit_d = results[-2]   # the 12th output is data misfit
    misfit_m = results[-1]  # the 13th output is data misfit
