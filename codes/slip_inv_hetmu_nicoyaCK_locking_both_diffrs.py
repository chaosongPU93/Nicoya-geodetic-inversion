...
def solveCoseismicInversion_TanhSlip(k, targets, m0_s_expr, mtrue_mu_expr_inv, gamma_val_H1, delta_val_L2, \
    ...
    if slip_transformer.has_any_bounds:
        ...
        # physical_slip_expr is a 2-component vector [strike, dip]                                                                                        
        m_strike_expr, m_dip_expr = ufl.split(physical_slip_expr)                                                                                         
        # Compute absolute dip slip for moment calculation that involves only dip-slip                                                                                                    
        s_dip_abs = abs(m_dip_expr)  # or ufl.conditional(m_dip_expr >= 0, m_dip_expr, -m_dip_expr)                                                       

    else:
        s_mag = ufl.sqrt(ufl.dot(m_fun, m_fun))
        m_strike_comp, m_dip_comp = ufl.split(m_fun)                                                                                                      
        s_dip_abs = abs(m_dip_comp)                                                                                                                       
    ...
    # if using only dip component of slip                                                                                                                     
    moment_dip = dl.assemble(m_mu_true * GPa2Pa * s_dip_abs * dS(fault))                                                                                  
    ...
    # calculate seismic potency, independent of the assumed elastic properties
    potency_dip = dl.assemble(s_dip_abs * dS(fault))
    ...
    if savefiles:
        ...
        outFileName = 'moment_dip_' + meshname + inv_str + mu_str_inv + '.txt'
        with open(resultpath + outFileName, 'w') as moment_file:
            moment_file.write(f"{moment:.6e} {M_w3:.4f} {potency:.6e} {moment_dip:.6e} {M_w3_dip:.4f} {potency_dip:.6e} \n")
...
if not BOUNDED:
    # gammas_s = [2.5e3]    #best
    # gammas_s = [1e2, 4e2, 6e2, 8e2, 1e3, 2e3, 4e3, 1e4, 5e4]
    # gammas_s = [1e2, 2.5e2, 5e2, 7.5e2, 1e3, 2.5e3, 5e3, 1e4, 5e4]
    # gammas_s = [5e1, 1e2, 2e2, 8e1]
    # gammas_s = [3e3]    #extra fining
    # gammas_s = [4e2, 6e2, 8e2, 1e3, 2e3, 3e3, 4e3, 1e4, 5e4]   # the complete set
    gammas_s = [4e1, 6e1, 8e1, 1e2, 1.8e2, 2e2, 2.2e2, 2.5e2, 3e2, 3.5e2, 4e2, 1e3, 1e4]

    #by 12/03, 1e2 is finished, but maybe too small
...
#     # file identifier
#     if BOUNDED:
#         inv_str = f"_lockbothbd_w{w_h}{w_v}_gs{gamma_s:.0e}_ds{delta_s:.0e}"
#     else:
...
# if BOUNDED:
#     outFileName = f"Lcurvelockbothbd_rs{rho_s:.0e}_{meshname}_{mu_str_inv}.txt"
# else:
#     outFileName = f"Lcurvelockboth_rs{rho_s:.0e}_{meshname}_{mu_str_inv}.txt"
# csvoutput = open(resultpath + outFileName, 'w+')
...
# print("All Het Inversion Finished!")   
...
for i in range(0, len(dmisfit_hom)):
    # csvoutput.write( "%.6e %.6e\n" %(dmisfit_hom[i], mmisfit_hom[i]) )
    csvoutput.write( "%.6e %.6e %.1e %.0e\n" %(dmisfit_hom[i], mmisfit_hom[i], gammas_s[i], rho_s) )
csvoutput.close()

print("All Hom Inversion Finished!")   