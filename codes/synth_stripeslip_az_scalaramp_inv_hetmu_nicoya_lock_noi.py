...
#   Forward μ: K_2LAYER heterogeneous.  Inversion L-curve: Het + Hom.
#
# Based on synth_stripeslip_az_inv_hetmu_nicoya_lock_noi.py (reference Az TanhSlip)
# and synth_stripeslip_az_scalaramp_inv_hetmu_uneven_nicoyaCK_lock_noi.py (AzSlip).
#
...
#   - Forward:     Het μ (K_2LAYER); call commented out (files from reference _noi.py)
#   - Inversion:   SCALAR amplitude a(x) ∈ [0, amp_max] via tanh; direction fixed to Az
#                  Run with both Het and Hom μ (_passes) to quantify mu-model effect
...
# Define sep
sep = "\n"+"#"*80+"\n"
print(sep, "START of Az scalar-amplitude synth recovery test — Het forward (nicoyaden_sm)", sep)
...
# %%
# Forward shear modulus (heterogeneous K_2LAYER for synthetics)
...
# (synth_stripeslip_az_inv_hetmu_nicoya_lock_noi.py with same parameters)
...
if not RUN_LCURVE:
    ...
    print(sep, "SINGLE TEST RUN — Az scalar amplitude inversion (Het μ)", sep)
...
# %%
# ============================================================
# L-curve: Het μ + Hom μ, Az scalar amplitude  (_passes)
# ============================================================
if RUN_LCURVE:
    ...
    _passes = [
        ("Het L-curve", gammas_s, outFileName_het, mtrue_mu_expr_het, mu_str_het),
        ("Hom L-curve", gammas_s, outFileName_hom, mtrue_mu_expr_hom, mu_str_hom),
    ]
    ...
    print("All Het+Hom Az scalar amplitude inversions finished!")
