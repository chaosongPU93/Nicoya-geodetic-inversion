...
def solveCoseismicInversion_Bounded(k, targets, m0_s_expr, mtrue_mu_expr_inv, gamma_val_H1, delta_val_L2, 
                            pollute=True, pollute_type='uniform', savefiles=True, verbose=True):
    """
    ENHANCED VERSION: Original solveCoseismicInversion_Bounded with optional bounded slip constraints
...
if slip_pattern == "checker" or slip_pattern == "stripe":
    ...
    # CONSTRAINT_METHOD = 'projected_cg'
    CONSTRAINT_METHOD = 'scipy_lbfgs'
else:
    # No constraints (original behavior)
    SLIP_BOUNDS = None
    CONSTRAINT_METHOD = 'unconstrained'
print(f"Using {CONSTRAINT_METHOD} with bounds: {SLIP_BOUNDS}")

mtrue_mu_expr_inv = mtrue_mu_expr_het
mu_str_inv = mu_str_het
print("Solving inverse problem based on: ", mu_str_inv)
mtrue_mu, xf, yf, zf, m, u, s_strike_fun, s_dip_fun, d_obs, d_cal, m_s_fault, misfitd, grad_m = \
    solveCoseismicInversion_Bounded(k, targets, m0_s_expr, mtrue_mu_expr_inv, gamma_val_H1, delta_val_L2, \
                            pollute=pollute, pollute_type=pollute_type, savefiles=False, verbose=True)
...
if SLIP_BOUNDS is not None:
    ...
    print(f"   Total violations: {strike_violations + dip_violations}")

    if strike_violations == 0 and dip_violations == 0:
        print("   ✅ ALL CONSTRAINTS SATISFIED!")
    else:
        print("   ⚠️ CONSTRAINTS VIOLATED!")

    print(f"🔍 FAULT-ONLY VALIDATION:")
    ...
    strike_ok = (min(m_sx_fault) >= SLIP_BOUNDS.get('strike_min', -np.inf) and
                max(m_sx_fault) <= SLIP_BOUNDS.get('strike_max', np.inf))
    dip_ok = (min(m_sy_fault) >= SLIP_BOUNDS.get('dip_min', -np.inf) and
            max(m_sy_fault) <= SLIP_BOUNDS.get('dip_max', np.inf))

    if strike_ok and dip_ok:
        print("   ✅ FAULT CONSTRAINTS SATISFIED!")
    else:
        print("   ⚠️ FAULT CONSTRAINTS VIOLATED!")
...
# print("Solving inverse problem based on: ", mu_str_inv)
# mtrue_mu, xf, yf, zf, m, u, s_strike_fun, s_dip_fun, d_obs, d_cal, m_s_fault, misfitd, grad_m = \
#     solveCoseismicInversion_Bounded(
...
#         constraint_method='projected_cg',  # or 'scipy_lbfgs' for robustness
#         pollute=pollute, pollute_type=pollute_type,
#         savefiles=False, verbose=True
#     )

# # Validate constraints were satisfied
# bounds_satisfied = validate_slip_bounds(m, slip_bounds, (xf, yf, zf))
...
# the current implementation allows slip everywhere on the fault interface without edge constraints.
# What This Means
# 
#   1. Fault edges: Slip can be non-zero all the way to the lateral boundaries of your fault interface
#   2. Trench: Slip can extend to the shallowest part of your fault (likely near the trench)
#   3. Fault bottom: Slip can be non-zero at the deepest extent of your fault
# 
# ### Example: Force zero slip at shallow depths (< 5 km depth)
# ```
# class ZeroSlipShallow(dl.SubDomain):
#     def inside(self, x, on_boundary):
#         return on_boundary and x[2] > -5000  # z > -5 km
# ```
# 
# ### Apply zero slip boundary condition
# ```
# zero_slip = dl.Constant((0.0, 0.0))
# shallow_bc = dl.DirichletBC(Vh[hp.PARAMETER], zero_slip, ZeroSlipShallow(), method='pointwise')
# ```
