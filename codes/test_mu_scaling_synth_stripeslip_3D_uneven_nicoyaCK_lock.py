...
def solveCoseismicForward(k, targets, mtrue_mu_expr_for, mtrue_s_expr=None, pollute=True, \
    ...
    if mtrue_mu_expr_for == None:
        ...
        # # NEW, using convex hull to avoid extrapolation artifacts, for 1D model, used layered values
        # vs_func, den_func, mu_func, _ = ut.process_velocity_models_hull(
        #     vel3d, vel1d, den1d, mesh, CG_mu_degree=CG_mu_deg, verbose=True)

        # # Above uses 1D layer look-up but pick the larger value across boundary, below uses smaller value
        # vs_func_l, den_func_l, mu_func_l, _ = ut.process_velocity_models_hull(
        #     vel3d, vel1d, den1d, mesh, CG_mu_degree=CG_mu_deg, verbose=True, side='left')

        # # Similar to above, but for 1D model, interpolate the layered values
        # vs_func1, den_func1, mu_func1, _ = ut.process_velocity_models_hull_interp(
        #     vel3d, vel1d, den1d, mesh, CG_mu_degree=CG_mu_deg, verbose=True)
        ...
        # # original, unamplified models
        # mtrue_mu_fun_ori = dl.Function(CG_mu)
        # new_values = mu_ref + 1.0 * (mu_func.vector()[:] - mu_ref)
        # mtrue_mu_fun_ori.vector()[:] = np.maximum(new_values, min_mu)

        # mtrue_mu_fun_ori1 = dl.Function(CG_mu)
        # mu_ref1 = np.mean(mu_func1.vector()[:])
        # new_values = mu_ref1 + 1.0 * (mu_func1.vector()[:] - mu_ref1)
        # mtrue_mu_fun_ori1.vector()[:] = np.maximum(new_values, min_mu)

        # ## original way of amplication, relative to global mean
        ...
        # mtrue_mu_fun1 = ut.scale_shear_modulus_by_1d(
        #     mu_func, vel1d, den1d, contrast_factor, min_mu=min_mu, verbose=True)

        # ## same way, but based on the interpolated 1D model, which is smoother and more physical than the layered values
        # mtrue_mu_fun2 = ut.scale_shear_modulus_by_1d(
        #     mu_func1, vel1d, den1d, contrast_factor, min_mu=min_mu, verbose=True)

        # ## new way of amplication, relative to 1D depth-layered values
        # mtrue_mu_fun1 = ut.scale_shear_modulus_by_1d(
        #     mu_func, vel1d, den1d, 2, min_mu=min_mu, verbose=True)
        
        # mtrue_mu_fun2 = ut.scale_shear_modulus_by_1d(
        #     mu_func, vel1d, den1d, 2.5, min_mu=min_mu, verbose=True)
        ...
        mtrue_mu_fun = ut.scale_shear_modulus_by_1d_interp(
            mu_func, vel1d, den1d, 1.0, min_mu=min_mu, verbose=True)  
        
        mtrue_mu_fun1 = ut.scale_shear_modulus_by_1d_interp(
            mu_func, vel1d, den1d, 2.5, min_mu=min_mu, verbose=True)    
    ...
    if savefiles:
        ...
        if isinstance(mtrue_mu_fun1, dl.Function):
            # m_mu_true_ori = mtrue_mu_fun_ori
            # m_mu_true_ori1 = mtrue_mu_fun_ori1
            m_mu_true = mtrue_mu_fun
            m_mu_true1 = mtrue_mu_fun1
            # m_mu_true2 = mtrue_mu_fun2
        ...
        # filename_base = resultpath + 'mu_true_test_ori' + meshname + mu_str_for
        # save_function_with_dofs(m_mu_true_ori, filename_base, 'shear modulus', cg_degree=CG_mu_deg)
        # print( m_mu_true_ori.vector().min(), m_mu_true_ori.vector().max() )

        # filename_base = resultpath + 'mu_true_test_ori1_' + meshname + mu_str_for
        # save_function_with_dofs(m_mu_true_ori1, filename_base, 'shear modulus', cg_degree=CG_mu_deg)
        # print( m_mu_true_ori1.vector().min(), m_mu_true_ori1.vector().max() )

        # filename_base = resultpath + 'mu_true_test_' + meshname + mu_str_for
        ...
        # filename_base = resultpath + 'mu_true_test1_' + meshname + mu_str_for
        # save_function_with_dofs(m_mu_true1, filename_base, 'shear modulus', cg_degree=CG_mu_deg)
        # print( m_mu_true1.vector().min(), m_mu_true1.vector().max() )

        # filename_base = resultpath + 'mu_true_test2_' + meshname + mu_str_for
        # save_function_with_dofs(m_mu_true2, filename_base, 'shear modulus', cg_degree=CG_mu_deg)
        # print( m_mu_true2.vector().min(), m_mu_true2.vector().max() )

        # filename_base = resultpath + 'mu_true_testfact2_' + meshname + mu_str_for
        # save_function_with_dofs(m_mu_true1, filename_base, 'shear modulus', cg_degree=CG_mu_deg)
        # print( m_mu_true1.vector().min(), m_mu_true1.vector().max() )

        # filename_base = resultpath + 'mu_true_testfact2.5_' + meshname + mu_str_for
        # save_function_with_dofs(m_mu_true2, filename_base, 'shear modulus', cg_degree=CG_mu_deg)
        # print( m_mu_true2.vector().min(), m_mu_true2.vector().max() )

        filename_base = resultpath + 'mu_true_testinterp1_' + meshname + mu_str_for
        save_function_with_dofs(m_mu_true, filename_base, 'shear modulus', cg_degree=CG_mu_deg)
        print( m_mu_true.vector().min(), m_mu_true.vector().max() )

        filename_base = resultpath + 'mu_true_testinterp2.5_' + meshname + mu_str_for
        save_function_with_dofs(m_mu_true1, filename_base, 'shear modulus', cg_degree=CG_mu_deg)
        print( m_mu_true1.vector().min(), m_mu_true1.vector().max() )
...
# if BOUNDED:
#     # gammas_s = [1e2]
#     # gammas_s = [1e1, 4e1, 6e1, 8e1, 1e2, 2e2, 4e2, 1e3, 1e4]
#     # gammas_s = [1e-1, 1e0, 1e1, 3e1, 1e2, 3e2, 1e3, 3e3, 1e4]   # suggested by chatGPT
#     # gammas_s = [5e1, 1e2, 2e2, 8e1]
#     # gammas_s = [3e2]
#     # gammas_s = [1.6e2, 1.8e2, 1.9e2, 2.2e2, 2.5e2]    # more extra fining!
#     # gammas_s = [1.8e2, 2.2e2, 2.5e2, 3.5e2]    # more extra fining!
#     gammas_s = [4e1, 6e1, 8e1, 1e2, 1.8e2, 2e2, 2.2e2, 2.5e2, 3e2, 3.5e2, 4e2, 1e3, 1e4]   # complete set

# print( sep, "Het L-curve computation with slip transformation", sep )
...
#     # file identifier
#     if BOUNDED:
#         inv_str = f"_synlockbd_w{w_h}{w_v}_gs{gamma_s:.1e}_ds{delta_s:.1e}"
#     else:
#         inv_str = f"_synlock_w{w_h}{w_v}_gs{gamma_s:.1e}_ds{delta_s:.1e}"
#     print("Inverse problem identifier: ", inv_str)

#     print(f"****** Computing solution with gamma_s = {gamma_s:.1e}, "
#         f"rho_s = {rho_s:.1e}, and delta_s = {delta_s:.1e} ******")

#     # Solve slip inverse problem within the heterogeneous shear modulus half-space WITH TANH TRANSFORMATION
#     mtrue_mu_expr_inv = mtrue_mu_expr_het
#     mu_str_inv = mu_str_het
#     # Set CG degree based on model type: CG2 for 3D model, CG1 (default) otherwise
#     if mtrue_mu_expr_inv is None:  # 3D heterogeneous model via process_velocity_models_hull
...
# if BOUNDED:
#     outFileName = f"Lcurvesynlockbd_rs{rho_s:.0e}_{meshname}_{slip_pattern}_{pattern_option}_{mu_str_for}_{mu_str_inv}.txt"
# else:
#     outFileName = f"Lcurvesynlock_rs{rho_s:.0e}_{meshname}_{slip_pattern}_{pattern_option}_{mu_str_for}_{mu_str_inv}.txt"
...
# print("All Het Inversion with Slip Bounds Finished!")
...
#     # ===== EXTRACT ONLY WHAT'S NEEDED =====
#     # Extract scalars immediately (avoid keeping large FEniCS objects)
#     misfit_d_hom = float(results[-2])   # misfitd (scalar)
#     misfit_m_hom = float(results[-1])   # grad_m norm or model misfit (scalar)
#     dmisfit_hom.append(misfit_d_hom)
#     mmisfit_hom.append(misfit_m_hom)
...
# # csvoutput = open(resultpath + outFileName + '_extra', 'w+') # make new and overwrite
# # csvoutput = open(resultpath + outFileName + '_extra', 'a') # append to existing file
# for i in range(0, len(dmisfit_hom)):
#     csvoutput.write( "%.6e %.6e %.1e %.0e\n" %(dmisfit_hom[i], mmisfit_hom[i], gammas_s[i], rho_s) )
# csvoutput.close()
