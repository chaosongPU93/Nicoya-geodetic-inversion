# %% [markdown]
# # Synthetic recovery test — Az-constrained SCALAR amplitude inversion (3D forward, CHECKERBOARD)
#
# Checkerboard variant of synth_stripeslip_az_scalaramp_inv_hetmu3D_uneven_nicoyaCK_lock_noi.py.
# Based on synth_checkslip_az_inv_hetmu3D_uneven_nicoyaCK_lock_noi.py (reference).
# Uses DeShon 3D velocity model (process_velocity_models_hull + scale_shear_modulus_by_1d)
# for both forward and inversion (CG2 elements). Forward skipped — files from _noi3D.py.
#
# KEY DIFFERENCE — inversion uses solveCoseismicInversion_AzSlip (Option A):
#   - True slip:   Az-directed checkerboard (FaultLocalCheckerboardAz), same as reference
#   - Forward:     3D μ (DeShon, CG2) — skipped, files already from _noi3D.py
#   - Inversion:   SCALAR amplitude a(x) ∈ [0, amp_max] via tanh; direction fixed to Az
#                  Run with both 3D Het mu and Hom mu to quantify mu-model effect
#   - Assessment:  recovery = rec/amp_plate (global, ∈[0,1]) + rec/true_amp (local, NaN on creeping)
#
...
def solveCoseismicForward(k, targets, mtrue_mu_expr_for, mtrue_s_expr=None, pollute=True,
    ...
    # NOTE: solveCoseismicForward does NOT support mtrue_mu_expr_for=None (3D case).
    # For 3D forward, use synth_stripeslip_az_inv_hetmu3D_uneven_nicoyaCK_lock_noi.py.
    if mtrue_mu_expr_for is None:
        raise ValueError("solveCoseismicForward does not support 3D (None) mu. "
                         "Use the non-scalar-amp 3D reference script for forward data.")
...
def solveCoseismicInversion_AzSlip(k, targets, m0_amp_expr, mtrue_mu_expr_inv,
    ...
    Recovery metric saved to: slip_recovery_<meshname><...>.txt
      columns: xf_m  yf_m  zf_m  true_amp_m  recovered_amp_m  recovery_ratio  recovery_local
      recovery_ratio = rec/amp_plate  ∈ [0,1] everywhere (global coupling coefficient)
      recovery_local = rec/true_amp   at locked patches only; NaN on creeping patches
    """
    BDM = dl.VectorFunctionSpace(mesh, "BDM", k)
    DGv = dl.VectorFunctionSpace(mesh, "DG", k-1)
    DGr = dl.VectorFunctionSpace(mesh, "DG", k-1)
    ...
    return mtrue_mu_fun, xf, yf, zf, m, u, m_phys_fun, s_strike_fun, s_dip_fun, \
...
# (synth_stripeslip_az_inv_hetmu3D_uneven_nicoyaCK_lock_noi.py with same parameters)
...
if not RUN_LCURVE:
    ...
    print(sep, "SINGLE TEST RUN — Az scalar amplitude inversion (3D)", sep)
    ...
    # Recovery ratio (global): recovered / plate_rate — coupling coefficient ∈ [0,1]
    # everywhere on fault; locked patches → 1, creeping patches → 0.
    recovery = m_amp_fault_t / amp
    print(f"  Amplitude recovery (all fault): [{recovery.min():.3f}, {recovery.max():.3f}]  (1.0 = fully locked)")

    # Recovery ratio (local): recovered / true_amp — only meaningful at locked patches.
    # NaN where true_amp ≈ 0 (creeping patches), since dividing by near-zero is undefined.
    eps = 0.01 * amp  # 1% of plate rate threshold
    valid = true_amp_arr > eps
    recovery_local = np.where(valid, m_amp_fault_t / np.where(valid, true_amp_arr, 1.0), np.nan)
    print(f"  Amplitude recovery (locked patches only): [{np.nanmin(recovery_local):.3f}, {np.nanmax(recovery_local):.3f}]")
...
if RUN_LCURVE:
    ...
    _passes = [
        ("Het L-curve (priority)", gammas_s_priority, outFileName_het, mtrue_mu_expr_het, mu_str_het + f"_CG{CG_mu_deg}", CG_mu_deg),
        ("Hom L-curve (priority)", gammas_s_priority, outFileName_hom, mtrue_mu_expr_hom, mu_str_hom,                    1),
        ("Het L-curve (rest)",     gammas_s_rest,     outFileName_het, mtrue_mu_expr_het, mu_str_het + f"_CG{CG_mu_deg}", CG_mu_deg),
        ("Hom L-curve (rest)",     gammas_s_rest,     outFileName_hom, mtrue_mu_expr_hom, mu_str_hom,                    1),
    ]
