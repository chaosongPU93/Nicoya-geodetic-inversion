# %% [markdown]
# # Synthetic forward (Az-consistent) + scalar amplitude inversion — STRIPE, Hom μ
#   nicoyaden_sm mesh (Slab2.0 plate interface), azi_equidist_proj coordinates.
#
# Based on: synth_stripe_az_consistfwd_nicoyaCK.py  (CF strategy)
#           synth_stripeslip_az_scalaramp_inv_hetmu_nicoya_lock_noi2.py  (nicoyaden_sm setup)
#
# PURPOSE: Run the AzSlip-consistent forward + scalar amplitude inversion on the
#          nicoyaden_sm mesh, so results can be compared against nicoyaCKden_une_sm.
#
...
#   Different from nicoyaCKden_une_sm which uses LL2ckmd with rot=45.
#
...
sep = "\n"+"#"*80+"\n"
print(sep, "START of Az-consistent forward + scalar amplitude inversion (stripe, Hom mu, nicoyaden_sm)", sep)
...
class PDEVarf_AzSlip_Direct:
    ...
    Use this for the AzSlip-consistent forward so forward and inversion operators match.
    """
...
# %%
# Forward shear modulus
mtrue_mu_expr_for = mtrue_mu_expr_hom
mu_str_for = mu_str_hom
print("Forward problem based on:", mu_str_for)
...
if not RUN_LCURVE:
    ...
    print(sep, "SINGLE TEST RUN — Az scalar amplitude inversion (consistent forward)", sep)
...
if RUN_LCURVE:
    ...
    outFileName = (f"Lcurvesynscaamp_cf_azbd_rs{rho_s:.0e}_{meshname}"
    ...
    _passes = [
        ("Hom L-curve (priority)", gammas_s_priority, outFileName, mtrue_mu_expr_hom, mu_str_hom),
    ]
