# %% [markdown]
# Az version of 'synth_checkslip_inv_hetmu3D_uneven_nicoyaCK_lock_noi.py'.
# Slip is prescribed in a fixed geographic azimuth direction (45° CW from North) using
# FaultLocalCheckerboardAz / FaultLocalStripesAz, with per-facet (c_strike, c_dip) computed
# so that the horizontal projection of the in-plane slip vector matches the prescribed azimuth.
# Inversion bounds updated for Az class on curved fault:
#   strike_bounds = (-amp, +amp)      # ±78.5 mm/yr; ±16 mm/yr clips 35% of truth
#   dip_bounds    = (0.0, 1.1*amp)   # 10% headroom above V_norm
# Single runs (Het + Hom) active; L-curve commented out.
...
if slip_pattern == "checker":
    ##### CHECKERBOARD SLIP MODEL — AZ CLASS #####
...
if not BOUNDED:
    if meshname == "nicoyaCK3":    # large fault extending to the whole subduction zone   
        ...
        # gamma_val_H1 = 1e3    # or this one, smoother
        # delta_val_L2 = 5e-6
    ...
    if meshname == "nicoyaCK4":     # small fault connecting trench
        ...
        # gamma_val_H1 = 1e1  
        gamma_val_H1 = 2.5e1  # best so far
        # gamma_val_H1 = 5e1  # or this one, smoother
        # gamma_val_H1 = 1e2  
        # gamma_val_H1 = 4e2
        # gamma_val_H1 = 1e3    
        # gamma_val_H1 = 1e4    
...
# # ===== RAM CLEANUP between Het and Hom runs =====
# del results
# gc.collect()
# try:
#     import psutil
#     mem_gb = psutil.Process().memory_info().rss / 1024**3
#     print(f"  Memory after Het cleanup: {mem_gb:.2f} GB")
# except ImportError:
#     pass
# # ================================================
...
if BOUNDED:
    # gammas_s = [1.5e2, 2.5e2, 3.5e2, 4.5e2]  # rough testing, even 1.5e2 too high
    # gammas_s = [4e1, 5e1, 6e1, 7e1, 8e1]  # more testing
    # gammas_s = [1.2e2, 1e2, 8e1]  # more testing
    # gammas_s = [1e2, 8e1]  # more testing
    gammas_s = [8e1]  # more testing
