...
# the goal is test whether the grain-like features in the interpolated 3D mu model can be smoothed out by 
# a higher-order CG element, e.g., CG2 instead of CG1 during the interpolation step.
# Previously, 'CG_mu = dl.FunctionSpace(mesh, "CG", 1)' was used in 'synth_stripeslip_inv_hetmu3D_nicoyaCK_lock_noi.py'.
# Here we will try out 'CG_mu = dl.FunctionSpace(mesh, "CG", 2)' instead.
# Since the goal is to test the effect of higher-order CG element for mu interpolation, forward modeling is enough
...
# NOTE: For uneven meshes, z-depths will be determined via ray casting after mesh loading.
# For even meshes, z=0 is used directly.
# We create placeholder arrays here that will be updated later for uneven meshes.
...
x_dense = x_2d
y_dense = y_2d
z_dense = np.zeros(n_2d)  # Placeholder, will be filled after mesh loading for uneven meshes

print(f"Dense grid: {len(lon_grid)} x {len(lat_grid)} = {n_2d} points")

# Create dense grid dataframe (z will be updated later for uneven meshes)
...
# Meshes with uneven top boundary, left at mean trench depth ~7 km, right at 0 km
...
if use_uneven_mesh:
    ...
    print(f"\nUpdated dense grid z-depths:")
    print(f"  z range: [{dense_data['z'].min():.3f}, {dense_data['z'].max():.3f}] km")
else:
...
def solveCoseismicForward(k, targets, mtrue_mu_expr_for, mtrue_s_expr=None, pollute=True, \
    ...
    if mtrue_mu_expr_for == None:
        ...
        vs_func, den_func, mu_func, _ = ut.process_velocity_models_hull(vel3d, vel1d, den1d, mesh, CG_mu_degree=CG_mu_deg, verbose=True)
...
def computeGridDisplacements(k, targets, mtrue_mu_expr_for, mtrue_s_expr=None, pollute=False, \
    ...
    if savefiles:
        # Save surface displacements in same format as GPS stations (with extrapolation for invalid points)
        ...
        if verbose:
            print(f"Saved {n_total_points} observations (including {n_total_points - n_valid_points} extrapolated)")
...
# Define the true model PARAMETERS for INVERSE problem
nu = 0.25
mtrue_mu_expr_het = None  # None triggers 3D velocity model via process_velocity_models_hull
