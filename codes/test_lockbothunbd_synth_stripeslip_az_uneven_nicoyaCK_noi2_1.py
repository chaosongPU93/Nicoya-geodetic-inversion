# %% [markdown]
# Duplicate of 'test_lockboth_synth_stripeslip_uneven_nicoyaCK_noi2.py' with two key changes:
#   1. Slip prescription uses FaultLocalStripesAz (create_fault_local_stripes_az) instead of
#      FaultLocalStripes (create_fault_local_stripes). The prescribed slip direction is fixed to
#      a geographic azimuth (default N45E = plate convergence direction) rather than the averaged
#      local down-dip direction. Per-facet (c_strike, c_dip) decomposition ensures the horizontal
#      projection of the in-plane slip vector is exactly the prescribed azimuth on every facet.
#   2. Inversion bounds updated to reflect the wider true strike range on the curved Nicoya fault:
#        strike_bounds = (-amp, +amp)      # full convergence rate; ±16 mm/yr clips 35% of truth
#        dip_bounds    = (0.0, 1.1*amp)   # 10% headroom above V_norm for fully-unlocked patches
#
# All other aspects (mesh, noise, regularization, inversion solver) are unchanged from reference.
...
# For easy testing, only Hom L-curve enabled (no Het L-curve). 
...
def solveCoseismicForward(k, targets, mtrue_mu_expr_for, mtrue_s_expr=None, pollute=True, \
    ...
    return mtrue, mtrue_mu, u_mtrue, xf, yf, zf, d_full, mtrue_s_fault
...
if slip_pattern == "checker":
    ##### CHECKERBOARD SLIP MODEL (Az version) #####
    print("Creating fault-local checkerboard (Az) with natural depth range...")
    ...
    mtrue_s_expr_gt = ut.create_fault_local_checkerboard_az(
        ...
        x0=x0,
        y0=y0,
        rotation_deg=pattern_rot_deg,
        azimuth_deg=slip_azimuth_deg,
        mesh_rotation_deg=rot,
        degree=5
    )

    slip_str_gt = (
        f"_check_x{x0/1e3:g}_y{y0/1e3:g}_dx{dx/1e3:g}_dy{dy/1e3:g}"
        f"_rot{pattern_rot_deg:g}_ms{amp:g}_az{slip_azimuth_deg:g}"
    )


elif slip_pattern == "stripe":
    ##### STRIPE SLIP MODEL (Az version) #####
    print("Creating fault-local stripes (Az) with natural depth range...")
    ...
    if pattern_option == 1:
        # 2-stripe, shallow-deep
        x_len = 80e3     # stripe width along dip direction
        y_len = 300e3    # stripe length along strike direction
        dx = 35e3        # gap between stripes
        stripe_spacing = x_len + dx
        x0 = 0;    y0 = -45e3
        pattern_rot_deg = 0.0

    elif pattern_option == 2:
        # 1-stripe, complementary pattern
        x_len = 40e3
        y_len = 300e3
        dx = 100e3
        stripe_spacing = x_len + dx
...
# %%
# # Solve forward problem to generate synthetics (must run: Az class produces different syndata)
...
# # Dense grid forward solve (for later plotting)
...
if BOUNDED:
    # Bounds for Az slip class on curved Nicoya fault:
    #   strike: ±amp  — analysis shows ±16 mm/yr clips 35% of truth in 10-30 km zone;
    #                   per-facet |c_strike| reaches ±amp on curved facets (std ~22 mm/yr)
    #   dip:    [-amp, amp]  — symmetric; curved fault Az prescription produces negative dip
    #                          on some facets, so one-sided [0, amp] would clip the truth
    # See notes/slip_prescription_and_pde_parameter_flow.md §5 for full analysis.
    if BOUND_TYPE == 'both':
        slip_transformer = SlipTransformation(
            strike_bounds=(-amp, amp),          # full convergence rate ±78.5 mm/yr
            # dip_bounds=(0.0, 1.1 * amp),        # 10% headroom above V_norm
            dip_bounds=(-amp, amp),        # 10% headroom above V_norm
        )
        print("Constraints to both strike and dip (Az physical bounds)")

    elif BOUND_TYPE == 'strike':
        slip_transformer = SlipTransformation(
            strike_bounds=(-amp, amp),
            dip_bounds=None,
        )
...
rho_s = 2.5e8   # allows variations of slip of the order of ~30 km 
gamma_val_H1 = 3e3  # used as of 12/10/2025, used gamma_val_H1:.1e
delta_val_L2 = gamma_val_H1 / rho_s 
...
# %%
# ### Single run — commented out; L-curve active ###
# # file identifier — '_testazbd' suffix marks Az-class bounds (±amp strike, 1.1*amp dip)
# if BOUNDED:
#     inv_str = f"_synlockbd_testazbd_w{w_h}{w_v}_gs{gamma_val_H1:.1e}_ds{delta_val_L2:.1e}"
# else:
#     inv_str = f"_synlock_testaz_w{w_h}{w_v}_gs{gamma_val_H1:.1e}_ds{delta_val_L2:.1e}"
...
# gammas_s = [4e1, 6e1, 8e1, 1e2, 1.8e2, 2e2, 2.2e2, 2.5e2, 3e2, 3.5e2, 4e2, 1e3, 1e4]
# gammas_s = [4e1, 8e1, 1e2, 4e2, 8e2, 2e3, 1e4]  # rough testing
# gammas_s = gammas_s[::-1]
# gammas_s = [1.5e2, 2e2, 2.5e2, 3e2, 3.5e2]  # rough testing

# gammas_s = [6e2, 8e2, 1e3]  # rough testing
# gammas_s = [1e3, 2e3, 3e3, 4e3]  # rough testing

gamma_s = 3e3
deltas_s = [3e-5, 1e-4, 3e-4]
...
for delta_s in deltas_s:
    rho_s = gamma_s  / delta_s

    # file identifier
    if BOUNDED:
        inv_str = f"_synlockbd_testazbd_w{w_h}{w_v}_gs{gamma_s:.1e}_ds{delta_s:.1e}"
    else:
        inv_str = f"_synlock_testaz_w{w_h}{w_v}_gs{gamma_s:.1e}_ds{delta_s:.1e}"
    print("Inverse problem identifier: ", inv_str)
...
if BOUNDED:
    outFileName = f"Lcurvesynlockbd_testazbd_gs{gamma_s:.0e}_{meshname}_{slip_pattern}_{pattern_option}_{mu_str_for}_{mu_str_inv}.txt"
else:
    outFileName = f"Lcurvesynlock_testaz_gs{gamma_s:.0e}_{meshname}_{slip_pattern}_{pattern_option}_{mu_str_for}_{mu_str_inv}.txt"
...
for i in range(0, len(dmisfit)):
    csvoutput.write( "%.6e %.6e %.1e %.0e\n" %(dmisfit[i], mmisfit[i], gamma_s, rho_s) )
csvoutput.close()
