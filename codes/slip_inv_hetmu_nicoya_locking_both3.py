...
# reverse direction????
data[cols_to_convert] = data[cols_to_convert] * (-1)
...
# # shear modulus for anomaly layer/wedge
# # mu_a = -0.3942 # ~25 GPa
# mu_a = mu_u
...
print( "The shear modulus for the upper plate mu = %.1f and lower plate mu = %.1f" %(mu_upper, mu_lower) )
# print( "The shear modulus for the anomaly layer is: mu = %.1f GPa while the upper plate mu = %.1f and lower plate mu = %.1f" %(mu_anomaly, mu_upper, mu_lower) )
...
inv_str = f"_revlockingboth_w{w_h}{w_v}_gs{gamma_val_H1:.0e}_ds{delta_val_L2:.0e}"
