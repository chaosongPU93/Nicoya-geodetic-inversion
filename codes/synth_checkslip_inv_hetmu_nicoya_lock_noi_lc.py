...
#
# * L-curve type, meant to run a few different gamma values with the same gamma/delta ratio=1e9, for 40 km slip patch size, add noise with real-data STD
...
def solveCoseismicForward(k, targets, mtrue_mu_expr_for, mtrue_s_expr=None, pollute=True, pollute_type='uniform', savefiles=True, verbose=True):
    ...
    if pollute:
        if pollute_type == "uniform":
            ...
            hp.Random().normal_perturb(noise_std_h, misfit.d)
...
def solveCoseismicInversion(k, targets, m0_s_expr, mtrue_mu_expr_inv, gamma_val_H1, delta_val_L2, pollute=True, pollute_type='uniform', savefiles=True, verbose=True):
    ...
    if pollute:
        if pollute_type == "uniform":
            ...
            ##### USE 'pointwiseStateObs_weights', but set 'weights' to 1
            weights = dl.Vector(MPI.COMM_WORLD, targets.shape[0]*15)
            obs_weights = np.zeros(targets.shape[0]*15,)
...
solveCoseismicForward(k, targets, mtrue_mu_expr_for, mtrue_s_expr=mtrue_s_expr_for, pollute=pollute, pollute_type=pollute_type, savefiles=True, verbose=True)
...
# solveCoseismicInversion(k, targets, m0_s_expr, mtrue_mu_expr_inv, gamma_val_H1, delta_val_L2, pollute=pollute, pollute_type=pollute_type, savefiles=True, verbose=True)
...
for gamma_s in gammas_s:
    ...
    # mu_str_inv = mu_str_het
    # print("Solving inverse problem based on: ", mu_str_inv)
    # solveCoseismicInversion(k, targets, m0_s_expr, mtrue_mu_expr_inv, gamma_s, delta_s, pollute=pollute, pollute_type=pollute_type, savefiles=True, verbose=True)
    # print("Het Inversion finished!!!")
